...
`XMLHttpRequest` -- это встроенный в браузер объект, который даёт возможность из JavaScript делать HTTP-запросы к серверу без перезагрузки страницы.
...
4. Слушайте события, чтобы получить ответ.
    ...
    xhr.onerror = function() { // происходит, только когда запрос не может быть выполнен в принципе
      alert(`Ошибка соединения`);
    };

    xhr.onprogress = function(event) { // отрабатывает периодически
...
```js
xhr.timeout = 10000; // время указывается в ms, итого 10 секунд
```
...
Звучит может быть и неплохо, но синхронные запросы используются редко, так как они блокируют выполнение JavaScript до тех пор, пока загрузка не завершена. В некоторых браузерах нельзя скроллить страницу, пока идёт синхронный запрос. Если же синхронный запрос по какой-то причине выполняется долго, браузер может предложить закрыть "зависшую" страницу.
