...
`XMLHttpRequest` -- это встроенный в браузер объект, который дает возможность из JavaScript делать HTTP-запросы к серверу без перезагрузки страницы.
...
В современной веб-разработке `XMLHttpRequest` можно использовать по трем причинам:

1. Cуществует много кода, использующего `XMLHttpRequest`, который нужно поддерживать.
2. Необходимость поддерживать старые браузеры и нежелание использовать полифиллы (например, чтобы уменьшить количество кода).
3. Потребность в функционале, который `fetch` пока что не может предоставить, к примеру, отслеживание прогресса загрузки.
...
1. Создать `XMLHttpRequest`.
    ```js
    let xhr = new XMLHttpRequest(); // аргументов нет
    ```
...
Заметьте, вызов `open`, вопреки своему названию, не открывает соединение. Он лишь конфигурирует запрос, но непосредственно отсылается запрос, только лишь после вызова `send`.
...
4. Слушать события, чтобы получить ответ.
    ...
    - `load` -- происходит, когда получен какой-либо ответ, в том числе и HTTP ошибки, например 404.
    - `error` -- когда запрос не может быть выполнен, например, нет соединения или невалидный URL.
    - `progress` -- происходит периодически во время загрузки ответа, сообщает о прогрессе.

    ```js
    xhr.onload = function() {
      alert(`Есть контакт: ${xhr.status} ${xhr.response}`);
    };

    xhr.onerror = function() { // происходит только когда запрос не может быть выполнен впринципе
      alert(`Ошибка соединения`);
    };

    xhr.onprogress = function(event) { // отрабатывает переодически
...
```js
xhr.timeout = 10000; // время указываем в ms, итого 10 секунд
```
...
Они существуют по историческим причинам, раньше с их помощью получали строки или XML-документы. Сегодня, стоит устанавливать желаемый тип объекта в `xhr.responseType`, и получать `xhr.response`, как показано выше.
```
...
Вы можете наткнуться на обработчики события `readystatechange` в реально старом коде, так уж сложилось исторически.

Сегодня, из-за существования событий `load/error/progress`, нет необходимости использовать устаревшее `readystatechange`.
...
Звучит может быть и неплохо, но синхронные запросы используются редко, так как они блокируют выполнение JavaScript до тех пор, пока загрузка не завершена. В некоторых браузерах нельзя скроллить страницу, пока идет синхронный запрос. Если же синхронный запрос по какой-то причине выполнется долго, браузер может предложить закрыть "зависшую" страницу.

Многие продвинутые возможности `XMLHttpRequest`, как выполнение запроса на другой домен или установление время ожидания, недоступны для синхронных запросов. Также, как вы могли заметить, ни о какой индикации прогресса речь тут не идёт.

Из-за всего этого, синхронные запросы стоит использовать весьма осмотрительно. Мы более не будем рассматривать их.
...
: Устанавливает заголовок `name` запроса со значением `value`.
    ...
    XMLHttpRequest не позволяется изменять их, ради безопасности пользователей и корректной работы запроса.
    ```
    ...
    Если заголовок определен, то его нельзя снять. Повторные вызовы лишь добавляют информацию к заголовку, а не перезаписывают его.
...
Итак, создаётся объект, по желанию наполняется данными из формы, затем, если нужно, с помощью метода `append` добавляются дополнительные поля, после чего:
...
Метод `.send(body)`достаточно всеяден. Он может отправить практически что угодно, включая объекты типа Blob и BufferSource.

## Прогресс загрузки

Событие `progress` работает только на стадии загрузки ответа.

А именно: если мы отправляем что-то через `POST` запрос, `XMLHttpRequest` сперва загрузит наши данные, а потом загрузит ответ сервера. И событие `progress` будет срабатывать только во вовремя загрузки ответа.

Если мы загружаем что-то большое, то нас однозначно больше интересует прогресс загрузки данных *на* сервер. Но событие `progress` тут бессильно.

Существует другой объект `xhr.upload`, без методов, только для событий загрузки.

Вот их список:

- `loadstart` -- начало загрузки данных.
- `progress` -- генерируется переодически во время загрузки.
...
xhr.upload.onprogress = function(event) {
  alert(`Загружено ${event.loaded} из ${event.total} байт`);
};
...
Пример из реальной жизни: загрузка файлов с индикатором прогресса:
...
`XMLHttpRequest` может выполнять кросс-доменные запросы, используя ту же политику в отношении CORS, как и [fetch](info:fetch-crossorigin).

Точно также как и при работе с `fetch`, не отсылаются куки и не происходит HTTP-авторизация по умолчанию на другом домене. Чтобы это изменить, установите `true` для свойства`xhr.withCredentials`:
...
xhr.send(); // в POST-запросе сюда можно передать данные как строку или объект FormData
...
- `timeout` -- запрос был отменён по причине истечения отведённого для него времени (происходит, только если это время было установлено).
...
Также мы уже видели событие: `readystatechange`. Исторически, оно появилось одним из первых, раньше даже чем была составлена спецификация. Сегодня нет необходимости его использовать, оно может быть заменено современными событиями, но на него можно часто наткнуться в старом коде.

Если же нам нужно следить именно за процессом загрузки данных на сервер, тогда можно использовать теже события, но для объекта `xhr.upload`.
