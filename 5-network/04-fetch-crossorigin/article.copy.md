# Fetch: Cross-Origin Запросы

Если мы сделаем `fetch` с произвольного вебсайта, это, вероятно, завершится неудачей.

Ключевой концепцией здесь является *origin* (источник) -- тройка домен/порт/протокол.

Cross-origin запросы -- запросы, отправленные на другой домен (или даже поддомен), или протокол, или порт -- требуют специальных заголовков от удалённой стороны. Эта политика называется "CORS": Cross-Origin Resource Sharing («совместное использование ресурсов между разными источниками»).
...
Потому что cross-origin ограничения защищают интернет от злых хакеров.

Серьёзно. Давайте сделаем краткое историческое отступление.

Многие годы JavaScript не имел никаких специальных методов для выполнения сетевых запросов.

**Скрипт с одного сайта не мог получить доступ к содержимому другого сайта.**

Это простое, но мощное правило было основой интернет-безопасности. Например, скрипт со страницы `hacker.com` не мог получить доступ к почтовому ящику пользователя на `gmail.com`. Люди чувствовали себя в безопасности.

Но веб-разработчики требовали больше власти. Чтобы обойти данное ограничение были придуманы разнообразные трюки.
...
- Таким образом было возможно сделать GET/POST запрос к другому сайту даже без сетевых методов.
...
Ещё один трюк заключался в использовании `<script src="http://another.com/…">` тэга. `script` мог иметь любой `src`, с любого домена. Но опять же -- было невозможно получить доступ к исходному содержимому этого скрипта.

Если `another.com` намеревался предоставить данные для такого доступа, тогда использовался так называемый "JSONP (JSON with padding)" протокол.

Вот последовательность:

1. Сначала мы зараранее объявляем глобальную функцию для приёма данных, например `gotWeather`.
...
4. По мере работы скрипта `gotWeather` выполняется, и, так как это наша функция, мы получаем данные.
...
Через некоторое время появились современные сетевые методы. Вначале cross-origin запросы были запрещены. Но в результате долгих дискуссий было разрешено делать кросс-доменные запросы таким способом, который не даёт каких-то новых возможностей, если это не было явно разрешено сервером.
...
**Основное отличие состоит в том, что "простой запрос" может быть сделан с `<form>` или `<script>`, без каких-то специальных методов.**

Таким образом, даже очень старый сервер должен быть готов принять простой запрос.
...
Когда мы пытаемся сделать непростой запрос, браузер посылает специальный "preflight" ("предполётный") запрос, который спрашивает у сервера -- согласен ли он принять такие cross-origin запросы или нет?

И, если сервер явно не даёт согласие в заголовках, непростой запрос не посылается.

Теперь мы углубимся в детали. Все они служат единственной цели -- гарантировать, что новые cross-origin возможности доступны только с явного согласия сервера.

## CORS для простых запросов

Если запрос cross-origin, браузер всегда добавляет к нему заголовок `Origin`.
...
Как вы можете видеть, `Origin` содержит именно origin (домен/протокол/порт), без пути.

Сервер может проверить `Origin` и, если он согласен принять такой запрос, добавить особый заголовок `Access-Control-Allow-Origin` к ответу. Этот заголовок должен содержать разрешённый origin (в нашем случае `https://javascript.info`), или звёздочку `*`. Тогда ответ успешен, в противном случае - ошибка.
...
2. Он проверяет правильность `Access-Control-Allow-Origin` в ответе, и если всё хорошо, тогда JavaScript получает доступ, в противном случае - запрещается c ошибкой.
...
По умолчанию при cross-origin запросе JavaScript может получить доступ только к "простым заголовка ответа":
...
Таким образом, если мы загружаем что-то и хотели бы отслеживать процент прогресса, то требуется дополнительное разрешение для доступа к этому заголовку (читайте ниже).
```

Чтобы предоставить JavaScript-у доступ к любому другому заголовку ответа, сервер должен перечислить его в заголовке `Access-Control-Expose-Headers`.
...
Поэтому, чтобы избежать недопониманий, браузер не делает "непростые" запросы (которые нельзя было сделать в старые времена) сразу. Перед этим он посылает предварительный, так назывемый "предполётный"("preflight") запрос, спрашивая разрешения.
...
- Коме того, заголовок `Access-Control-Max-Age` может указывать количество секунд для кэширования разрешений. Так что браузеру не придётся посылать preflight для последующих запросов, удовлетворяющих данным разрешениям.
...
Есть три причины того, почему этот запрос не является простым (достаточно одной):
- Метод `PATCH`
...
- Особые cross-origin заголовки:
    - `Origin` -- origin источника.
...
Это разрешило бы будущюю коммуникацию, в противном случае возникает ошибка.
...
Кроме того, preflight ответ кэшируется на время, указанное в заголовке `Access-Control-Max-Age` (86400 секунд, один день), так что последующие запросы не вызовут preflight. Они будут посланы сразу при условии, что они соответствуют разрешениям.
...
Основной запрос имеет заголовок `Origin` (потому что он cross-origin):
...
Сервер не должен забывать о добавлении `Access-Control-Allow-Origin` к ответу. Успешный preflight не осовбождает от этого:
...
## Учётный данные (credentials)

Cross-origin запрос по умолчанию не содержит никаких учётных данных (куки или HTTP аутентификация).
...
Потому что запрос с учётными данными намного более мощный, чем анонимный. Если он разрешён, то это даёт JavaScript полную возможность действовать и получать доступ к конфиденциальной информации от имени пользователя.

Сервер действительно настолько верит страницам с `Origin`? Запросу с учётными данными нужен дополнительный заголовок, чтобы пройти.

Чтобы включить учётные данные, нам нужно добавить опцию `credentials: "include"`, например так:
...
Если сервер желает принять запрос с учётными данными, он должен добавить заголовок `Access-Control-Allow-Credentials: true` к ответу, в дополнение к `Access-Control-Allow-Origin`.
...
Пожалуйста, обратите внимание: в `Access-Control-Allow-Origin` запрещено использовать звёздочку `*` для запросов с учётными данными. Там должен быть именно origin, как показано выше. Это дополнительная мера безопасности, чтобы гарантировать, что сервер действительно знает, кому он доверяет.


## Итог

Сетевые методы разделяют cross-origin запросы на два вида: "простые" и все остальные.
...
Существенное отличие заключается в том, что простые запросы выполнялись с древних времён с использованием тэгов `<form>` или `<script>`, в то время как непростые долгое время были невозможны для браузеров.
...
- → Браузер посылает заголовок `Origin` с origin.
...
- ← Для запросов с учётными данными, сервер должен установить:
...
- → Браузер посылает запрос `OPTIONS` на тот же url с заголовками:
