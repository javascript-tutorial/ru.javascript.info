...
let promise = fetch(url, {
  ...
  referrer: "about:client", // "" в случае no-referrer, или URL текущего origin
...
Опцию `signal` мы объяснили в <info:fetch-прерывание запроса>.
...
В заголовке указывается URL-адрес страницы, с которой пришел запрос. В большинстве случаев, это не играет такой важной роли для передачи информации, однако, в некоторых случаях, из-за угрозы безопасности, адрес лучше убрать или модифицировать.  

**Опция `referrer` позволяет установить любой` Referer` в пределах текущего origin или же отключить его.**
...
Для того, чтобы установить другой URL-адрес для текущего origin:

```js
fetch('/страница', {
  // предположим, что мы находимся на странице https://javascript.info
  // мы можем установить любое значение Referer, при условии что оно принадлежит текущему origin
*!*
  referrer: "https://javascript.info/другаястраница"
...
- **`"origin"`** -- отправлять в `Referer` только текущий origin, а не полный URL-адрес страницы, например, послать `http://site.com`, вместо `http://site.com/path`.
- **`"origin-when-cross-origin"`** -- отправлять полный referrer для запросов в пределах текущего origin, но для кроссдоменных запросов отправлять только само значение origin
- **`"same-origin"`** -- отправлять полный referrer для запросов в пределах текущего origin, а для кроссдоменных запросов не отправлять referer вообще.
- **`"strict-origin"`** -- отправлять только значение origin, не отправлять referrer для HTTPS→HTTP запросов.
- **`"strict-origin-when-cross-origin"`** -- для запросов в пределах текущего origin отправлять полный referrer, для кроссдоменных запросов отправлять только значение origin, в случае HTTPS→HTTP запросов не отправлять ничего. 
...
В противном случае, если мы хотим, чтобы удалённая сторона знала домен откуда поступает запрос, мы можем отправить только часть «origin» URL-адреса:
...
- **`"omit"`** -- не отправлять ни при каких обстоятельствах, даже для запросов, сделанных в пределах текущего origin.
...
Обычно, когда документ выгружен, все связанные с ним сетевые запросы прерываются. Но опция `keepalive` указывает браузеру выполнять запрос в фоновом режиме даже после того, как он покидает страницу. Поэтому важно, чтобы наш запрос был успешным.
