...
Существует другой объект `xhr.upload`, без методов, только для событий отправки.

Список событий похож на события `xhr`, но все они относятся к процессу отправки на сервер:
