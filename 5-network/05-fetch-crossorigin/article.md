...
- `Origin` содержит имя сервера, откуда будет производиться загрузка.
...
- `Access-Control-Allow-Origin`  должен содержать заголовок ответа о том, может ли ответ сервера быть доступен коду из запроса `Origin`
