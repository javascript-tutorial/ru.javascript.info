...
Для отправки, `socket.send(body)` принимает строки или данные в любом другом бинарном формате, включая `Blob`, `ArrayBuffer` и другие. Дополнительных настроек не требуется, просто отправьте отправьте их.
