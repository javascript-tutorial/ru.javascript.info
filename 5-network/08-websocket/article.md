...
**Метод WebSocket `.send()` может отправлять и текстовые и двоичные данные.**
...
**При получении данных, текст всегда поступает в виде строки. А для двоичных данных мы можем выбрать один из двух форматов: `Blob` или `ArrayBuffer`.**

Значение по умолчанию `socket.bufferType` -- `"blob"`, поэтому двоичные данные поступают в Blobs.

[Blob](info:blob) -- это высокоуровневый двоичный объект, он напрямую интегрируется с `<a>`, `<img>` и другими тегами. Это значение по умолчанию. Но для обработки данных, если требуется доступ к отдельным байтам, мы можем изменить его на `"arraybuffer"`:
...
2. Для каждого принятого websocket: `clients.add(socket)` и добавить обработчик `onmessage` для приёма сообщений.
