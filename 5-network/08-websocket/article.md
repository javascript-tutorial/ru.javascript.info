...
socket.onclose = function(event) {
  if (event.wasClean) {  
    ...
    alert('[закрыто] Коннект отключен -> Соединение прервано');
  }
};
...
Для отправки, `socket.send(body)` принимает строки или данные в любом другом бинарном формате, включая `Blob`, `ArrayBuffer` и другие. Дополнительных настроек не требуется, просто отправьте отправьте их [строки/данные].
...
Вы также можете скачать его (верхняя правая кнопка в ифрейм) и запустить локально. Просто не забудьте установить [Node.js](https://nodejs.org/en/) и выполнить команду `npm install ws` до запуска.
