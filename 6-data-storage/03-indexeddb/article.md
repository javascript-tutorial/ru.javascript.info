...
У разных браузеров и пользователей на уровне ОС есть свое собственное независимое хранилище.
```
...
1. Прослушиватель `db.onversionchange` сообщает нам о попытке параллельного обновления, если текущая версия базы данных устарела.
2. Прослушиватель `OpenRequest.onblocked` сообщает нам об обратной ситуации: в другом месте есть соединение с устаревшей версией, и оно не закрывается, поэтому новое соединение установить невозможно.
...
Сначала давайте разберемся с первым типом поиска: по ключу.
