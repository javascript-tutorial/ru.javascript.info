...
- Хранилище ключей/значений (key/value): доступны несколько типов ключей, а значения могут быть (почти) любыми.
- Поддерживает транзакции для надёжности.
- Поддерживает запросы в диапазоне ключей и индексов.
...
Мы также можем использовать `async/await` с помощью обёртки, которая основана на промисах, например <https://github.com/jakearchibald/idb>. Это очень удобно, но обёртка не идеальна, она не может заменить события на все случаи, поэтому мы начнём с событий, а затем будем использовать обёртку.

## Открыть базу данных

Чтобы начать работать с IndexedDB нам необходимо открыть базу данных.
...
У нас может быть множество баз данных с различными именами, определённых в пределах текущего проекта (домен/протокол/порт). Таким образом, разные сайты не могут получить доступ к базам данных друг друга.

После открытия базы данных нам необходимо назначить обработчики событий для объекта `openRequest`:
- `success`: база данных готова к работе, для дальнейшей работы с базой данных нужно использовать объект `openRequest.result`.
- `error`: не удалось открыть базу данных.
- `upgradeneeded`: устарела версия базы данных (смотрите ниже).

**IndexedDB имеет встроенный механизм "версионирования", отсутствует в серверных базах данных.**

IndexedDB является клиентской базой данных и в отличие от серверных, у нас нет прямого доступа к данным. Но когда мы публикуем новую версию нашего приложения, возможно нам необходимо обновить базу данных.
...
openRequest.onupgradeneeded = function() {
  let db = openRequest.result;
  switch(db.version) { // получить версию БД
    case 0:
...
Хранилище объектов - это основная концепция IndexedDB. В других базах данных это "таблицы" или "коллекции". Здесь хранятся данные. В базе данных может быть множество хранилищей: одно для пользователей, другое для товаров и так далее.

Не смотря на то, что название "хранилище объектов", примитивы тоже могут там храниться.
...
**Должен быть уникальный `ключ` для каждого значения в хранилище.**     

Ключ должен быть одним из следующих типов: number, date, string, binary или array. Это уникальный идентификатор объекта: по ключу мы можем искать/удалять/обновлять значения.

![](indexeddb-structure.png)

Мы можем указать ключ при добавлении значения в хранилище, аналогично `localStorage`. Это удобно для хранения примитивных значений. Но когда мы храним объекты, IndexedDB позволяет установить свойство объекта в качестве ключа, что гораздо удобнее. Или мы можем автоматически сгенерировать ключи.
...
- `keyOptions` - это необязательный объект с одним или двумя свойствами:
  ...
  - `autoIncrement` -- если `true`, то ключ будет формироваться автоматически для новых объектов, как постоянно увеличивающееся число (increment).

Если при создании хранилища не указать `keyPath`, то нам потребуется явно указать ключ позже, при сохранении объекта.
...
Это техническое ограничение. Вне обработчика мы сможем добавлять/удалять/обновлять данные, но хранилища объектов изменяются только во время обновления версии базы данных.

Есть два основных способа обновить версию:
1. Мы можем сравнить версии, затем выполнить операции для каждой версии.
2. Или мы можем взять список существующих хранилищ объектов, используя `db.objectStoreNames`. Этот объект является [DOMStringList](https://html.spec.whatwg.org/multipage/common-dom-interfaces.html#domstringlist), в нём есть метод `contains(name)`, используя который можно проверить существование хранилища. Тогда мы можем обновляться в зависимости от того, какие хранилища существуют.
...
Определение "транзакция" является общим, транзакции используются во многих видах баз данных.
...
1. Вычесть деньги со счёта.
2. Добавить предмет в инвентарь.

Было бы довольно плохо, если бы мы успели завершить первую операцию, а затем что-то пошло бы не так, например отключили электричество, и мы не смогли завершить вторую операцию. Обе операции должны быть успешно завершены (покупка завершена, хорошо!) или необходимо отменить обе операции(по крайне мере, чтобы пользователь сохранил свои деньги и мог повторить).

Транзакции гарантируют это.

**Все операции с данными должны выполняться внутри транзакций в IndexedDB.**
...
- `type` – тип транзакции, один из:
  ...
  - `readwrite` -- только чтение и запись, изменение хранилищ объектов недоступно.

Существует также тип транзакции `versionchange`: такие транзакции могут делать всё, но мы не можем создавать их вручную. IndexedDB автоматически создаёт транзакцию `versionchange`, когда открывает базу данных для обработчика `updateneeded`. Вот почему это единственное место откуда мы можем обновить структуру базы данных, создать/удалить хранилища объектов.

```smart header="Для чего нужны типы транзакций?"
Производительность является причиной, почему транзакции необходимо помечать как `readonly` или `readwrite`.

Большинство `readonly` транзакций могут одновременно работать с одним и тем же хранилищем объектов, а `readwrite` транзакции не могут. `readwrite` транзакции "блокируют" хранилище от записи. Следующая транзакция должна дождаться выполнения предыдущей, перед тем как получит доступ к тому же самому хранилищу.
```
...
4. ...Обработать результат запроса `(4)`, выполнить другие запросы при необходимости и так далее.
...
Так же, как при открытии базы данных, мы отправляем запрос: `books.add(book)` и после ожидаем события `success/error`.

- `request.result` для `add` является ключом нового объекта.
- Ошибка находится в `request.error` (если есть).

## Автозавершение транзакций

В примере выше мы запустили транзакцию и выполнили запрос `add`. Мы могли бы выполнить больше запросов. Как нам успешно завершить ("commit") транзакцию?

Короткий ответ: нам не нужно вручную подтверждать транзакцию.
...
```smart header="Что такое \"пустая очередь микрозадач\"?"
Очередь микрозадач описана в [другой главе](info:async-await#microtask-queue). Короче говоря, пустая очередь микрозадач означает, что для всех выполненных промисов выполнены их обработчики `.then/catch/finally`.

Другими словами, обработка завершенных промисов и выполнение "awaits" выполняется перед завершением транзакции.

Это мелкая техническая деталь. Если мы используем `async/await` вместо базовых промисов, то можно предположить, что транзакция завершится после выполнения всех её запросов, после этого текущий код завершится.
```

Таким образом, в приведённом выше примере не требуется специального кода, чтобы завершить транзакцию.

В принципе "автозавершения транзакций" есть важный побочный эффект. Мы не можем вставить асинхронную операцию как `fetch` или `setTimeout` в середине транзакции. IndexedDB не заставит транзакцию ожидать их выполнения.

В приведённом ниже коде `request2` в строке с `(*)` ошибка, потому что транзакция уже завершена, больше нельзя выполнить в ней запрос:
...
Всё потому, что `fetch` является асинхронной операцией, макрозадача. Транзакции завершаются раньше, чем браузер приступает к выполнению макрозадач.

Авторы спецификации IndexedDB считают, что транзакции должны быть недолговечными. Из соображений производительности.

Особенно `readwrite` транзакции "блокируют" хранилища от записи. Таким образом, если одна часть приложения инициирует `readwrite` транзакцию в хранилище объектов `books`, то другая часть приложения, которая хочет сделать то же самое, должна ждать: новая транзакция "зависает" до завершения первой. Это может привести к странным задержкам, если транзакции слишком долго выполняются.

Так что делать?

В приведённом выше примере мы могли бы запустить новую транзакцию `db.transaction` перед новым запросом `(*)`.

Но было бы ещё лучше, если бы мы выполнили эти операции вместе, в рамках одной транзакции, чтобы разделить транзакции IndexedDB от "других" асинхронных операций.

Сначала сделаем `fetch`, подготовим данные, если нужно, затем создадим транзакцию и выполним все запросы к базе данных.

Чтобы поймать момент успешного выполнения, мы можем повешать обработчик на событие `transaction.oncomplete`:
...
Только `complete` гарантирует, что транзакция сохранена целиком. По отдельности запросы могут выполниться, но в последней операции может пойти что-то не так (например ошибка чтения/записи).
...
Метод производит отмену всех изменений и генерирует событие `transaction.onabort`.
...
Мы должны это ожидать, не только потому, что мы можем допустить ошибку, но и по причинам, которые не связаны с транзакцией. Например, размер хранилища может быть превышен. Таким образом, мы должны быть готовы обработать такой случай.

**Запрос, выполненный с ошибкой, автоматически отменяет транзакцию и отменяет все изменения.**

Иногда запрос может завершиться с некритической ошибкой. Мы бы хотели обработать ошибку в `request.onerror` и продолжить выполнение транзакции. Тогда, чтобы предотвратить отмену транзакции нам следует использовать `event.preventDefault()`.

В примере ниже новая книга добавлена с одинаковым ключом (`id`). Метод `store.add` генерирует в этом случае ошибку `"ConstraintError"`. Мы обрабатываем её без отмены транзакции:
...
request.onerror = function(event) {
  // ConstraintError появляется, когда происходит добавление объекта с уже существующим ключом
...
**Пузырь события IndexedDB: `запрос` -> `транзакция` -> `база данных`.**

Все события являются DOM событиями с возможностью перехвата и со всплытием, но обычно используется только всплытие.
...
Есть две основные функции поиска в хранилище объектов:
1. По ключу или по диапазону ключей. То есть: по `book.id` в хранилище "books".
2. По полям объекта, например `book.price`. Сначала необходимо добавить индекс для полей, по которым будет поиск.

Сначала давайте разберёмся с ключами и диапазоном ключей `(1)`.

Методы поиска поддерживают либо точные ключи, либо так называемые "запросы диапазона" -- [IDBKeyRange](https://www.w3.org/TR/IndexedDB/#keyrange) объекты, которые задают "диапазон ключей".
...
- `IDBKeyRange.bound(lower, upper, [lowerOpen], [upperOpen])` означает: между `lower` и `upper`, включительно, если передать в соответствующий `open` true.
...
- `store.getKey(query)` -- поиск первого ключа, который удовлетворяет запросу, обычно используется диапазон.
- `store.getAllKeys([query], [count])` -- поиск всех ключей, которые удовлетворяют запросу, обычно используется диапазон, возможно ограничить поиск, передав `count`.
- `store.count([query])` -- получить общее количество ключей, которые удовлетворяют запросу, обычно используется диапазон.
...
Для поиска по другим полям объекта нам нужно создать дополнительную структуру данных с именем "индекс" (index).
...
Сначала нам нужно создать индекс. Индексы должны создаваться в `upgradeneeded`, как хранилище объектов:
...
Метод `delete` удаляет значения по запросу, как в `getAll`.
...
Если нам надо удалить книги основываясь на цену или на любое другое поле, сначала нам надо найти ключ в индексе, а затем выполнить `delete`:
...
***cursor* является особенным объектом, который позволяет перемещаться по хранилищу объектов с заданным запросом (query) и возвращает одну пару ключ/значение в единицу времени, что позволяет экономить память.**

Так как хранилище объектов внутренне отсортировано по ключу, курсор проходит по хранилищу по порядку хранения ключей (по возрастанию по умолчанию).
...
- **`direction`** необязательный аргумент, доступные значения:
  ...
  - `"nextunique"`, `"prevunique"` -- то же самое, курсор проходит только по уникальным ключам (работают только с индексами, например для нескольких книг с price=5, результатом будет только первая).

**Основным отличием курсора является то, что `request.onsuccess` генерируется несколько раз: один раз для каждого результата.**
...
- `advance(count)` -- продвинуть курсор `count`, пропустить значения.
- `continue([key])` -- продвинуть курсор к следующему значению в диапазоне соответсвия или к следующему ключу.

Независимо от того, есть ли ещё значения, соответствующие курсору или нет - вызывается `onsuccess`, затем в` result` мы можем получить курсор, указывающий на следующую запись или на `undefined`.

В приведённом выше примере курсор был создан для хранилища объектов.

Но мы также можем создать курсор для индексов. Как мы помним, индексы позволяют искать по полю объекта. Курсоры для индексов работают так же, как для хранилищей объектов -- они позволяют экнономить память, возвращая одно значение в единицу времени.

Для индекс курсоров `cursor.key` является ключом индекса (например price), нам следует использовать свойство `cursor.primaryKey` как ключ объекта:
...
Итак, у нас есть весь сахар для "простого асинхронного кода" и для "try..catch".

### Обработка ошибок

Если мы не перехватим ошибку, то она, как обычно, провалится.
...
### Ловушка "Неактивная транзакция"

Как мы уже знаем, транзакции автоматически фиксируются, как только браузер завершает работу с текущим кодом и микрозадачами. Поэтому, если мы поместим *микрозадачу* наподобие `fetch` в середину транзакции, транзакция не будет ожидать её завершения. Произойдёт автозавершение транзакции. Поэтому, следущий запрос не выполнится.

Для обёртки над промисом и `async/await` поведение такое же.
...
Решение такое же, как работа с обычным IndexedDB: либо создать новую транзакцию, либо разделить задачу на части.
...
Это работает в большинстве случаев. Примеры на странице библиотеки <https://github.com/jakearchibald/idb>.

В нескольких редких случаях, когда нам нужен оригинальный объект `request`, мы можем получить в нему доступ используя свойство `promise.request`:

```js
let promise = books.add(book); // получаем промис (не ожидаем результата)
...
IndexedDB можно рассматривать как "localStorage на стероидах". Это простая ключ-значение (key-value) база данных, достаточно мощная для оффлайн приложений, но простая в использовании.
...
1. Используйте обёртку над промисами, например [idb](https://github.com/jakearchibald/idb).
2. Открыть базу данных: `idb.openDb(name, version, onupgradeneeded)`
    - Создание хранилищей объектов и индексов происходит в обработчике `onupgradeneeded`.
