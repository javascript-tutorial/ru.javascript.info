...
У нас может быть множество баз данных с различными именами, определённых в контексте текущего проекта (домен/протокол/порт). Таким образом, разные сайты не могут получить доступ к базам данных друг друга.
...
**IndexedDB имеет встроенный механизм "версионирования", который отсутствует в серверных базах данных.**

В отличие от серверных баз данных IndexedDB работает на стороне клиента, и у нас нет прямого доступа к данным. Но когда мы публикуем новую версию нашего приложения, то возможно, что нам необходимо обновить базу данных.
...
Другими словами, обработка завершенных промисов и выполнение "awaits" выполняются перед завершением транзакции.
...
Транзакции с автофиксацией имеют важный побочный эффект. Мы не можем вставить асинхронную операцию как `fetch` или `setTimeout` в середине транзакции. IndexedDB не заставит транзакцию ожидать их выполнения.
...
Авторы спецификации IndexedDB из соображений производительности считают, что транзакции должны быть недолговечными.
...
Итак, у нас всё готово для асинхронного кода и "try..catch".

### Обработка ошибок

Если мы не перехватим ошибку, то она, как обычно, пробросится дальше.
...
IndexedDB можно рассматривать как "localStorage на стероидах". Это простая ключ-значение база данных, достаточно мощная для оффлайн приложений, но простая в использовании.
