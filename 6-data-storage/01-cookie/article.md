...
Домен определяет, где доступен файл cookie. Однако на практике существуют определенные ограничения. Мы не можем указать здесь какой угодно домен.

**Нет никакого способа разрешить доступ к файлам cookie из другого домена 2-го уровня, поэтому `other.com` никогда не получит куки, установленный по адресу `site.com`.**

Это ограничение безопасности, позволяющее нам хранить конфиденциальные данные в файлах cookie, которые должны быть доступны только на одном сайте.

По умолчанию куки доступно лишь тому домену, который его установил.

Пожалуйста, обратите внимание, что по умолчанию файл cookie также не передается поддомену, например `forum.site.com `.

```js
// если мы установим файл cookie на веб-сайте site.com...
document.cookie = "user=John"

// ...мы не увидим этого на forum.site.com
alert(document.cookie); // нет user
```

...Но это можно изменить. Если мы хотим разрешить такие поддомены, как `forum.site.com` чтобы получить набор файлов cookie в `site.com `, это возможно.

Чтобы это произошло, при установке файла cookie в `site.com `, мы должны явно установить параметр `domain` для корневого домена: `domain=site.com `. После этого все поддомены увидят такой файл cookie.
