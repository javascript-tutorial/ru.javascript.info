...
Как мы видим, интерфейс похож на Map, но также запоминается и порядок элементов, можно получить n-й ключ при помощи `.key(n)`.
...
Нам достаточно находится на том же источнике (домен/протокол/порт), при этом URL-путь может быть разным.
...
1. Если ключ генерируется пользователем, то он может быть каким угодно, включая `length` или `toString`, а также другой встроенный метод `localStorage`. В этом случае `getItem/setItem` сработают нормально, а вот чтение/запись как свойства объекта не подойдут:
    ```js run
    let key = 'length';
