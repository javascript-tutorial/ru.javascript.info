...
...Но если Вы откроете ту же страницу в другой вкладке, и повторите попытку, код выше вернет `null`, что значит "ничего не найдено".
...
- Событие срабатывает на всех объектах `window`, где доступно хранилище, а также там, где оно было вызвано (на вкладке для `sessionStorage`, глобально для `localStorage`).
