...
Протокол `WebSocket` (стандарт [RFC 6455](https://datatracker.ietf.org/doc/html/rfc6455)) предназначен для решения любых задач и снятия ограничений обмена данными между браузером и сервером.
...
Sec-WebSocket-Key
: Случайный ключ, который генерируется браузером: 16 байт в кодировке [Base64](https://ru.wikipedia.org/wiki/Base64).
...
Полное описание протокола содержится в [RFC 6455](https://datatracker.ietf.org/doc/html/rfc6455).
...
Например, для Node.JS одной из самых известных библиотек является [Socket.IO](https://socket.io).
