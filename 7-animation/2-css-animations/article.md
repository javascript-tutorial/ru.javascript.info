...
- Браузернные инструменты разработчика также имеют специальную поддержку для создания кривых Безье в CSS:
    ...
    2. Выберете вкладку `Elements`, затем обратите внимание на под-панель `Styles` на правой стороне.
    3. Свойства CSS со словом `cubic-bezier` будут иметь иконку перед этим словом.
    4. Кликните по иконке для того чтобы редактировать вашу кривую.
...
Div `#digit` имеет фиксированную ширину и границу, поэтому он выглядит как красное окно.

Мы сделаем таймер: цифры будут появляться одна за другой, дискретно.

Чтобы добиться этого, мы скроем `#stripe` за пределами `#digit`, используя `overflow: hidden`, а затем шаг за шагом сдвинем `#stripe` влево.
...
Щелчок по цифре немедленно изменяет ее на `1` (первый шаг), а затем изменяется в начале следующей секунды.
