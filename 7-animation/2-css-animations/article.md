...
JavaScript может быть использован для управления CSS-анимациями и улучшать их, использую совсем маленькие кусочки кода

## CSS переходы [#css-transitions]

Идея CSS переходов проста: мы выбираем свойство и описываем как его изменения должны быть анимированы. Когда свойство меняется, браузер анимирует это изменение.
...
Теперь, если элементу установлен класс `.animated`, любое изменение свойства `background-color` будет анимироваться 3 секунды.
...
Мы рассмотрим их все через мгновение, но давайте отметим сейчас, что общее свойство `transition` позволяет объявить их вместе в последовательности: `property duration timing-function delay`, и также анимировать несколько свойств одновременно.
...
Теперь давайте рассмотрим свойства анимации каждое в отдельности.

## transition-property

В свойство `transition-property` записывается список свойств для изменения, например: `left`, `margin-left`, `height`, `color`.

Не все свойства могут быть анимированы, но [многие из них](http://www.w3.org/TR/css3-transitions/#animatable-properties-). Значение свойства `all` означает "анимируй все свойства".
...
Свойство `transform` анимируется вот так:
...
Мы можем начать анимацию "с середины", с определенного числа, например, используя негативное значение `transition-delay` соответствующие необходимому числу.
...
На первый взгляд это очень сложное свойство, но оно становится простым, если уделить ему немного времени.
...
...Как мы видим это прямая линия. Значению времени (`x`) соответствует, значение изменения свойство (`y`) которое равномерно анимируется от `0` до `1`.

В примере ниже поезд движется слева направо с одинаковой скоростью (нажмите на поезд):
...
Есть несколько встроенных кривых: `linear`, `ease`, `ease-in`, `ease-out` и `ease-in-out`.
...
<style>
*!*
  @keyframes go-left-right {        /* присваивает имя: "go-left-right" */
...
- JavaScript анимации гибче. В них может присутствовать любая анимационная логика, как например "взорвать" элемент.
