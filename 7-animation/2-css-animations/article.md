...
JavaScript может быть использован для управления такими CSS. Это позволяет делать более сложные анимации, используя небольшие кусочки кода.
...
Все что нам нужно, это изменить свойство, а дальше браузер сделает плавный переход сам.
...
Теперь, если элементу присвоен класс `.animated`, любое изменение свойства `background-color` будет исполняться в течение трёх секунд.
...
Далее мы рассмотрим их все, а сейчас ещё заметим, что есть также общее свойство `transition`, которое позволяет задать их одновременно в последовательности `property duration timing-function delay` и анимировать несколько свойств одновременно.

Например, у этой кнопки анимируется два свойства `color` и `font-size` одновременно:
...
Теперь рассмотрим каждое свойство анимации.
...
- Ось `x` -- это ось времени: `0` -- начало анимации, `1` -- конец анимации, соответствующая значению в свойстве `transition-duration`.
...
...И как же можно показать замедление поезда?
