...
1. Мы соеденяем их, чтобы получить N-1 отрезков.
...
```online
Если в описании алгоритма есть что-то неясное, то приведённые выше примеры показывают, как строится кривая.
```
...
Иногда перед нами стоит другая задача: нарисовать кривую *через* несколько точек, чтобы все они были на одной гладкой кривой. Эта задача называется [интерполяцией](https://en.wikipedia.org/wiki/Interpolation), и здесь мы её не охватываем.
...
Учитывая координаты контрольных точек <code>P<sub>i</sub></code>: первая контрольная точка имеет координаты <code>P<sub>1</sub> = (x<sub>1</sub>, y<sub>1</sub>)</code>, вторая: <code>P<sub>2</sub> = (x<sub>2</sub>, y<sub>2</sub>)</code> и т.д., координаты кривой описываются уравнением, зависящим от параметра `t` на отрезке `[0,1]`.
