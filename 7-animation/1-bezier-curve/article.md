...
На самом деле это очень простая вещь, которую стоит изучить один раз, а затем чувствовать себя комфортно в мире векторной графики и продвинутых анимаций.
...
3. Параметр `t` пробегает значения от `0` до `1`. В примере выше использован шаг `0.05` т.е. в цикле `0, 0.05, 0.1, 0.15, ... 0.95, 1`.
    ...
    - На каждом из <span style="color:#825E28">коричневых</span> отрезков берётся точка, находящаяся от начала на расстоянии от 0 до `t` пропорционально длине. Так как отрезков – два, то и точек две штуки.
...
| ![](bezier3-draw1.png)   | ![](bezier3-draw2.png) |
    ...
    получаем точку в конце первой четверти отрезка, для t=0.5 (второй рисунок) – в середине отрезка. На рисунках выше эта точка отмечена красным.
...
5. По мере того как `t` пробегает последовательность от `0` до `1`, каждое значение `t` добавляет к кривой точку. Совокупность таких точек для всех значений образуют кривую Безье. Это <span style="color:red">красный</span> и параболический на картинках выше.
...
1. Мы соеденяем их что бы получить N-1 сегментов.
2. Затем для каждого `t` от `0` до `1`, мы берем точку на каждом отрезке на расстоянии пропорциональном `t` и соединяем их. Там будет N-2 сегментов.
3. Повторяем 2 шаг пока не останется одна точка.

Эти точки делают кривую.

```online
**Запускайте и приостанавливайте примеры чтобы ясно увидеть сегменты и то как строится кривая.**
```
...
```online
Если в описании алгоритма есть что-то неясное, то приведенные выше примеры показывают, как строится кривая.
```

Поскольку алгоритм является рекурсивным, мы можем построить кривые Безье любого порядка, то есть: используя 5, 6 или более контрольных точек. Но на практике многие пункты менее полезны. Обычно мы берем 2-3 точки, а для сложных линий склеиваем несколько кривых. Это проще для разработки и расчета.

```smart header="Как нарисовать кривую *через* заданные точки?"
Мы используем контрольные точки для кривой Безье. Как видим, они не на кривой, кроме первой и последней.

Иногда перед нами стоит другая задача: нарисовать кривую *через* несколько точек, чтобы все они были на одной гладкой кривой. Эта задача называется [интерполяцией](https://en.wikipedia.org/wiki/Interpolation), и здесь мы ее не охватываем.
...
Как мы видели -- на самом деле нет необходимости это знать, большинство людей просто рисуют кривую перемещая точки с помощью мыши. Но если вы увлекаетесь математикой -- вот она.

Учитывая координаты контрольных точек <code>P<sub>i</sub></code>: первая контрольная точка имеет координаты <code>P<sub>1</sub> = (x<sub>1</sub>, y<sub>1</sub>)</code>, вторая: <code>P<sub>2</sub> = (x<sub>2</sub>, y<sub>2</sub>)</code>, и т.д., координаты кривой описываются уравнением зависящим от параметра `t` от отрезка `[0,1]`.
...
Теперь, когда `t` работает от `0` до `1`, набор значений `(x, y)` для каждого `t` образует кривую для таких контрольных точек.
...
2. Использование процесса рисования: алгоритм де Кастельжау.

Их удобство в том, что:

- Мы можем рисовать плавные линии с помощью мыши, перемещаясь вокруг контрольных точек.
