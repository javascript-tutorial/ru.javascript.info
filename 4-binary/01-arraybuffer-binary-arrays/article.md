...
- `Uint8Array`, `Uint16Array`, `Uint32Array` -- целые беззнаковые числа по 8, 16 и 32 бита соответственно.
  - `Uint8ClampedArray` -- 8-битные целые, обрезаются по верхней границе при присвоении (об этом ниже).
...
// а сейчас мы получим 16-битное число на той же позиции 0, оно состоит из 2-х байт, число из обоих равно 65535
...
dataView.setUint32(0, 0); // установим 4-знаковое число в 0, при этом 0 запишется в 4 первых байта
```
...
- Это может быть типизированный массив`TypedArray`:
    ...
    - `Uint8ClampedArray` -- для 8-битных целых, которые обрезаются по верхней границе при присвоении.
