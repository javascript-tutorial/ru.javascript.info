# ArrayBuffer, массивы двоичных данных

В сфере веб-разработки мы встречаемся с двоичными (бинарными) данными чаще всего тогда, когда требуется выполнить какие-то действия над файлами (создать, загрузить или скачать). Другим типичным примером такой встречи является обработка изображений.
...
- **`Uint8Array`** -- представляет каждый байт в `ArrayBuffer` как отдельное число; возможные значения находятся в промежутке между 0 и 255 (в байте 8 бит, отсюда такой набор). Такие значения называются "8-битное целое без знака".
- **`Uint16Array`** -- представляет каждые 2 байта в `ArrayBuffer` как целое число; возможные значения находятся в промежутке между 0 и 65535. Такие значения называются "16-битное целое без знака".
- **`Uint32Array`** -- представляет каждые 4 байта в `ArrayBuffer` как целое число; возможные значения находятся в промежутке между 0 и 4294967295. Такие значения называются "32-битное целое без знака".
...
*!*
let view = new Uint32Array(buffer); // представим содержимое как последовательность 32-битных целых чисел без знака
...
Представления, о которых речь шла ранее (`Uint8Array`, `Uint32Array` и т.д.), наследуют от [TypedArray](https://tc39.github.io/ecma262/#sec-typedarray-objects). У них имеется набор одинаковых свойств и методов.
...
1. Если передан аргумент типа `ArrayBuffer`, то создаётся объект-представление. Мы уже использовали этот синтаксис ранее.
...
Что если мы попытаемся записать в типизированный массив значение, разрядность которого превышает допустимую для данного массива? Ошибки не будет. Лишние биты просто будут отброшены.
...
`Uint8ClampedArray`, упомянутый ранее, ведёт себя по-другому в данных обстоятельствах. В него записываются значения 255 для чисел, которые больше 255, и 0 для отрицательных чисел. Это полезно при обработке изображений.
...
- В случае типизированных массивов при вызове соответствующего конструктора формат данных строго задавался. Весь массив состоял из однотипных значений. Доступ к i-ому элементу можно было получить как `arr[i]`.
...
- **`byteLength`** -- длина данных, используемых в представлении (по умолчанию - до конца `buffer`).
...
// а сейчас мы получим 16-битное число на той же позиции 0, это теперь 2 байта с максимальными значениями
...
dataView.setUint32(0, 0); // 0 запишется в 4 первых байта
```

Представление `DataView` отлично подходит, когда мы храним данные в разных форматах в одном буфере. Например, мы храним последовательно пары значений, одно из которых 16-битное целое, а другое -- 32-битное с плавающей точкой. `DataView` позволяет легко организовать к ним доступ.
...
В большинстве случаев мы создаём типизированные массивы и работаем с ними, не обращая внимание на `ArrayBuffer`. Мы можем получить к нему доступ с помощью `.buffer` и при необходимости создать другое представление.

Существуют ещё 2 дополнительных термина:
...
Эти обозначения используются в описаниях методов, которые работают с двоичными данными. `BufferSource` встречается очень часто и означает "бинарные данные в любом виде" -- `ArrayBuffer` или его представление.
