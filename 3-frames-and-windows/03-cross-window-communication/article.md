...
- если у нас есть ссылка на другой объект `window`, например, на всплывающее окно, созданное с помощью `window.open` или на `window` из `<iframe>` и все они соответствуют политике "Same Origin", тогда у нас будет полный доступ к ним.
...
Сначала мы посмотрим страницы, которые имеют одинаковый источник и не конфликтуют с политикой "Same Origin", а после этого разберём способ отправки сообщений между окнами, который позволяет обойти политику "Same Origin".
