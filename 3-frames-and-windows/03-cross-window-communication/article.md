...
- если у нас есть ссылка на другой объект `window`, например, на всплывающее окно, созданное с помощью `window.open` или на `window` из `<iframe>` и все они соответствуют политике "Одинакового источника", тогда у нас будет полный доступ к ним.
- в противном случае, если они имеют другие источники, доступ к контенту будет ограничен: к переменным, к объекту `document` и ко всему остальному. Единственное исключение - объект `location`: его можно изменять (таким образом перенаправляя пользователя). Но нельзя *читать* `location` (нельзя узнать, где находится пользователь, чтобы не было никаких утечек информации).

Сначала мы посмотрим страницы, которые имеют одинаковый источник и не конфликтуют с политикой "Одинакового источника", а после этого разберём способ отправки сообщений между окнами, который позволяет обойти эту политику.

````warn header="Поддомены могут иметь одинаковый источник"
Есть небольшое исключение в политике "Одинакового источника".
...
Свойство `iframe.onload` на самом деле то же самое, что и `iframe.contentWindow.onload`. Оно сработает, когда встроенное окно полностью загрузится со всеми ресурсами.

...Но `iframe.onload` сработает всегда, в то время как `iframe.contentWindow.onload` требует выполнения политики "Одинакового источника".
```
...
...Событие `onload` сработает, когда ифрейм загрузится полностью со всеми ресурсами. Но что если мы хотим запустить код раньше, например, на событие `DOMContentLoaded` встроенного документа?

Это невозможно, если ифрейм из другого источника. Но если источники совпадают, мы можем попробовать поймать момент, когда появляется новый документ, и после этого добавить необходимые обработчики:
...
<script>
  ...
  let timer = setInterval(() => {
    ...
    // когда документ изменился, установим обработчики
...
- `window.frames` -- коллекция "дочерних" windows (для вложенных фреймов).
...
Атрибут `sandbox` позволяет исключить определённые действия внутри `<iframe>`, чтобы предотвратить выполнение ненадежного кода. Атрибут помещает ифрейм в "песочницу", отмечая его как имеющий другой источник и/или накладывая на него дополнительные ограничения.

По умолчанию для `<iframe sandbox src="...">` список ограничений такой же, как для ифрейма. В атрибут можно передать список "исключений", разделённый пробелами, которые не нужно применять к ифрейму: `<iframe sandbox="allow-forms allow-popups">`. Ограничения из списка не будут применены.
...
`targetOrigin`
: Определяет источник для окна-получателя, так что только окно из данного источника может получать сообщение.
...
Если определить `targetOrigin`, тогда мы можем быть уверены, что окно получит данные только в том случае, если мы до сих пор на сайте. Хорошо, когда при отправке данных это учитывается.

Например, здесь `win` получит сообщения только в том случае, если документ из источника `http://example.com`:
...
Чтобы вызвать метод или получить контент из другого окна, нам необходимо иметь ссылку на него.
...
Если окна имеют одинаковые источники (протокол, домен, порт), то они могут делать друг с другом всё, что угодно.
