...
- `window.frames` -- коллекция "дочерних" windows (для вложенных ифреймов).
...
Это способ обойти политику "Одинакового источника". Он позволяет обмениваться информаций, скажем `john-smith.com` и `gmail.com`, но только в том случаее, если оба сайта согласны и вызывают сооветствующиее JavaScript функции. Это делает общение безопасным для пользователя.
...
Если определить `targetOrigin`, тогда мы можем быть уверены, что window получит данные только в том случае, если мы до сих пор на сайте. Хорошо, когда при отправке данных это учитывается.
...
Чтобы добавить обрабочик, следует использовать метод `addEventListener`, короткий синтаксис `window.onmessage` не работает.
...
Исключения:
- Окна, которые имеют общий домен второго уровня: `a.site.com` и `b.site.com`. И установленное свойство `document.domain='site.com'` в обоих окнах переведёт их в состояние "Одинакового истоника".
