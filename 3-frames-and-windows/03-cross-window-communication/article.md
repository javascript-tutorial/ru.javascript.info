...
По умолчанию для `<iframe sandbox src="...">` список ограничений такой же, как для ифрейма. В атрибут можно передать список "исключений", разделенный пробелами, которые не нужно применять к ифрейму: `<iframe sandbox="allow-forms allow-popups">`. Ограничения из списка не будут применены.
