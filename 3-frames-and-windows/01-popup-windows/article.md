...
Попапы существуют с доисторических времен. Они были придуманы для отображения нового контент поверх главного окна, оставив главное окно открытым. Но с тех пор появились другие способы сделать это: Javascript может отправлять запросы к серверу, поэтому попапы стали использоваться все реже и реже. Однако иногда они могут быть действительно удобны. 

В прошлом "злые" вебсайты заваливали посетителей всплывающими окнами. "Нехорошие" страницы могли открывать сотни попапов с рекламой. Поэтому теперь большинство браузеров пытаются заблокировать всплывающие окна, чтобы защитить пользователя.
...
Если вдуматься, то такое поведение не очень удобно. Если код непосредственно указан в обработчике `onclick`, то все просто. Но что, если попап должен открываться не сразу, и например, код находится внутри `setTimeout`? 
...
...а этот пример сработает и в Firefox:
...
Мы получили два разных результата из-за того, что Firefox считает таймаут в 2000мсек или менее допустимым, но все, что свыше -- не вызывающим доверия и происходящим вне ведома пользователя. Поэтому попап из первого примера будет заблокирован, а из второго -- нет.
...
Важно помнить, что содержимое и настройки внешнего `document` доступны только для таких окон, URL которых происходит из того же источника (англ. – «Same Origin»), т.е. совпадают домен, протокол и порт.
...
Этот код открое и затем закроет окно:
...
Теоретически, существуют методы `window.focus()` и `window.blur()` для установки попап в фокус или потери им фокуса. Также существуют события `focus/blur`, которые позволяют установить попап в фокус и отследить, когда фокус переводится на какое-то другое окно. 

На "плохих" сайтах это также являлось способом манипуляции. Например:

```js run
window.onblur = () => window.focus();
```

Когда пользователь пытается перевести фокус на другое окно, этот код возвращает фокус назад. Таким образом, фокус как бы "блокировался" в "нехорошем" попапе.

Из-за этого в браузерах появились ограничения, которые препятствуют такого рода поведению кода. Есть множество ограничений для защиты пользователя от назойливой рекламы и "плохих" страниц, которые зависят от конкретного браузера.

Например, мобильный браузер обычно полностью игнорирует такие вызовы метода. Также фокусировка не работает, когда попап открыт в отдельной вкладке, а не в новом окне.

Но все-таки иногда методы фокусировки бывают полезны. Например:

- Когда мы открываем попап, может быть хорошей идеей запустить для него `newWindow.focus()`. Для некоторых комбинаций браузера операционной системы это позволит нам быть уверенными, что пользователь заметит это новое окно.
- Если нужно отследить, когда посетитель использует веб-приложение, можно отслеживать `window.onfocus/onblur`. Это позволяет ставить на паузу и продолжать выполнение анимаций и других интерактивных действий на странице. Но важно помнить, что событие `blur` означает, что пользователь покинул окно, но при этом он может по-прежнему видеть его. 
...
А также, если мы открываем попап, хорошей практикой будет уведомить пользователя об этом. Иконка с открывающимся окном поможет пользователю пережить перевод фокуса в новое окно и не забыть, что главное окно по-прежнему открыто. 
