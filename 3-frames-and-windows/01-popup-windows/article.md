...
... и откроется новое окно с указанным URL. В большинстве современных браузеров по умолчанию будет открываться новая вкладка вместо отдельного окна.
...
params
: Строка параметров для нового окна. Содержит настройки, разделённые запятыми. В ней не должно быть пробелов. Например `width:200,height=100`.
...
Теоретически, есть методы `window.focus()` и `window.blur()`, чтобы навести/снять фокус. Также существуют события `focus/blur`, которые позволяют отследить, когда фокус переводится на какое-то другое окно.
...
Когда пользователь пытается снять фокус, происходит вызов `window.onblur` и фокус возвращается назад. Таким образом, фокус как бы "блокируется" в попапе, который не нужен пользователю.
...
Например, мобильные браузеры обычно полностью игнорируют вызовы `window.focus()`. Также фокусировка не будет работать, когда попап открыт в отдельной вкладке (в отличие от открытия в отдельном окне).
...
- Попап может быть открыт с помощью вызова `open(url, name, params)`. Метод возвращает ссылку на это новое окно.
- По умолчанию браузеры блокируют вызовы `open`, выполненные не в результате действий пользователя. Обычно всплывает уведомление, чтобы пользователь мог дать разрешение на вызов этого метода.
- Вместо попапа открывается вкладка, если в вызове `open` не указаны его размеры.
- У попапа есть доступ к родительскому окну через свойство `window.opener`.
