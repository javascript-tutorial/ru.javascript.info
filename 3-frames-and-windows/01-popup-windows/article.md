...
Чтобы предотвратить злоумышленное использование, браузер обычно блокирует эти методы. Они гарантированно работают только с попапами, которые мы открыли сами и у которых нет дополнительных вкладок.
```

```warn header="No minification/maximization"
Методами JavaScript нельзя уменьшить или увеличить размер окна. За это отвечают функции уровня операционной системы, и они скрыты от фронтенд-разработчиков.

Методы перемещения и изменения размера окна не работают для увеличенных/уменьшенных окон.
```
...
`win.scrollBy(x,y)`
: Прокрутить окно на `x` пикселей вправо и `y` пикселей вниз относительно текущего положения скрола. Допустимы отрицательные значения.
