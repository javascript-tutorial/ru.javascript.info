...
````smart header="`Элемент группировки элемента формы как \"подформа\""
...
Для элемента `<select>` мы также можем получить индекс выбранного пункта через `select.selectedIndex`, либо используя коллекцию пунктов `select.options`. Полное описание этого и других элементов находится по адресу <https://html.spec.whatwg.org/multipage/forms.html>.
