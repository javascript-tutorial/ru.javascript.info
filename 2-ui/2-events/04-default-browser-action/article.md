...
Есть некоторые события, как `touchmove` на мобильных устройствах (когда пользователь перемещает палец по экрану), которое по умолчанию запускает скроллинг, но мы можем отменить это действие, используя `preventDefault()` в обработчике.
...
<button *!*oncontextmenu="alert('Рисуем наше меню'); return false"*/!*>
  клик правой кнопкой мыши для вызова нашего контекстного меню
</button>
```
...
<p>клик правой кнопкой мыши для контекстного меню документа</p>
<button id="elem">клик правой кнопкой мыши для контекстного меню кнопки</button>
...
```html autorun height=80 no-beautify run
<p>клик правой кнопкой мыши для меню документа</p>
<button id="elem">клик правой кнопкой мыши для меню кнопки(добавлен event.stopPropagation)</button>
...
<button id="elem">клик правой кнопкой мыши для меню кнопки</button>
...
Также, если мы рассматриваем пример с тегом `<a>`, тогда обратите внимание: браузер предоставляет возможность открывать ссылки в новом окне (кликая правой кнопкой мыши или используя другие возможности). И пользователям это нравится. Но если мы заменим ссылку кнопкой и стилизуем её как ссылку, используя CSS, то специфичные функции браузера для тега `<a>` всё равно работать не будут.
```
