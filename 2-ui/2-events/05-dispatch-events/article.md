...
- *Тип события* -- любая строка типа `"click"` или же что-то придуманное нами -- `"hey-ho!"`.
...
Для своих событий стоит использовать конструктор `CustomEvent`. У него есть дополнительная опция `detail`, с помощью которой можно передавать дополнительную информацию для нашего события. После чего все обработчики смогут получить к ней доступ через `event.detail`.
