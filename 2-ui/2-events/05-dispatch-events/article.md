...
- *options* -- объект с двумя не обязательными свойствами:
...
Обычно события обрабатываются асинхронно. Это значит, что если браузер обрабатывает `onclick` и где-то по пути произойдёт новое событие, то оно ждёт пока закончится обработка `onclick`.
