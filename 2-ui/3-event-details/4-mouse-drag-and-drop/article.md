...
1. Когда посетитель нажимает кнопку (`mousedown`) -- запомним расстояние от курсора до левого верхнего угла мяча в переменных `shiftX/shiftY`. Далее будем удерживать это расстояние при перетаскивании.
