...
Раньше в JavaScript существовало ограничение: зарезервированное слово типа `"class"` не могло быть свойством объекта. Это ограничение сейчас не существует, но в то время было невозможно иметь `"class"` свойтво, как `elem.class`.
...
Если мы присваиваем что-то `elem.className`, то это заменяет всю строку с классами. Иногда это то, что нам нужно, но часто мы хотим добавить/удалить (add/remove) один класс.
...
Кроме того, `classList` является итерабельным, поэтому мы можем перечислить все классы так:
...
Если мы установим в `display` пустую строку, то браузер применит CSS классы и его встроенные стили, как если бы такого `display` свойства вообще не было.
...
<script>
  // здесь мы даже можем устананливать специяальные флаги для стилей, например, "important"
...
1. *Вычисленное* (computed) значение – это то, которое получено после применения всех правил CSS и CSS-наследования. Например, `height:1em` или `font-size:125%`.
...
Например, если есть свойства `paddingLeft/paddingTop`, то что мы получим вызывая `getComputedStyle(elem).padding`? Ничего, или, может быть, "сгенерированное" значение из известных падингов? Стандарта для этого нет.
