...
Иначе говоря, результат такой же, как при вызове `elem.querySelectorAll(css)[0]`, который сначала найдет *все* элементы, а потом возьмёт первый, а `elem.querySelector` находит только первый. Так писать быстрее и короче.
...
*Предки* элемента - родитель, родитель родителя, его родитель и так далее. Вместе они образуют цепочку иерархии от элемента до вершины DOM.
