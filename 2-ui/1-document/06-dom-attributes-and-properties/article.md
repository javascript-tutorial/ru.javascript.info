...
Ранее мы уже видели встроенные DOM свойства. Их много. Но технически нас никто не ограничивает, и если этого мало -- мы можем добавить свое собственное свойство.

DOM узлы -- это обычные JavaScript объекты. Мы можем их изменять.
...
Также мы можем модифицировать встроенные прототипы, такие как `Element.prototype`, и добавлять новые методы ко всем элементам:
...
Итак, DOM свойства и методы ведут себя так же, как и обычные JavaScript объекты:
...
В HTML у тегов могут быть атрибуты. Когда браузер парсит HTML, чтобы создать DOM объекты для тегов, он распознает *стандартные* атрибуты и создает DOM свойства для них.

Таким образом, когда у элемента есть `id` или другой *стандартный* атрибут, создается соответствующее свойство. Но этого не происходит, если атрибут нестандартный.
...
Вот расширенная демонстрация работы с атрибутами:
...
В примере ниже `id` модифицируется как атрибут, и мы можем увидеть, что свойство также изменено. То же самое работает и в обратную сторону:
...
Если нам нужно значение `href` или любого другого атрибута в точности, как оно написано в HTML, мы можем использовать `getAttribute`.


## Нестандартные атрибуты, dataset

При написании HTML мы используем много стандартных атрибутов. Но что насчет нестандартных, пользовательских? Во-первых, давайте посмотрим, полезны они или нет? Для чего они нужны?
...
Но с пользовательскими атрибутами могут возникнуть проблемы. Что если мы используем нестандартный атрибут для наших целей, а позже он появится в стандарте и будет выполнять какую-то функцию? Язык HTML живой, он растет, появляется больше атрибутов, чтобы удовлетворить потребности разработчиков. В этом случае могут возникнуть неожиданные эффекты.

Чтобы избежать конфликтов, существуют [data-*](https://html.spec.whatwg.org/#embedding-custom-non-visible-data-with-the-data-*-attributes) атрибуты.

**Все атрибуты начинаются с префикса "data-" и зарезервированы для использования программистами. Они доступны в свойстве `dataset`.**

Например, Если у `elem` есть атрибут с названием `"data-about"`, он доступен так `elem.dataset.about`.
...
Для большинства нужд DOM свойства могут сослужить нам хорошую службу. Мы должны использовать атрибуты только тогда, когда DOM свойства нас не устраивают, когда нам нужны именно атрибуты, например:

- Нам нужен нестандартный атрибут. Но если он начинается с `data-`, тогда мы должны использовать `dataset`.
