...
Здесь мы увидем, как создавать новые элементы "на лету" и изменять уже существующие.
...
Это был пример HTML. Теперь давайте создадим такой же `div`, используя JavaScript (при условии, что стили всё ещё в HTML или во внешнем css файле).
...
Для этого есть специальным метод: `document.body.appendChild(div)`.
...
Эти методы "старой школы": они существуют с древних времён и мы можем встретить их во многих старых скриптах. К несчастью, есть некоторые задачи, которые трудно решить с ними.

Например, как вставить *html*, если элемент у нас в виде строки? Или, если у нас есть узел, как вставить *другой* перед ним? Конечно, всё это выполнимо, но не элегантно.

Поэтому существует два других набора с методами вставки, на все случаи.

### prepend/append/before/after

Этот набор методов предоставляет более гибкие вставки:
...
Небольшое изображение, как методы работают:
...
Первый параметр - это строка, указывающая куда будет произведена вставка. Значение должно быть одним из следующих:

- `"beforebegin"` -- вставляет `html` до `elem`,
- `"afterbegin"` -- вставляет `html` в `elem` в начало,
- `"beforeend"` -- вставляет `html` в `elem` в конец,
- `"afterend"` -- вставляет `html` после `elem`.

Второй параметр - это HTML-строка, вставляется "как есть".
...
Это как мы можем вставлять произвольный HTML на нашу страницу.
...
Они существуют в основном, чтобы сделать синтаксис "единым". На практике часто используется только `insertAdjacentHTML`. Потому что для элементов и текста у нас есть методы `append/prepend/before/after` -- их быстрее написать и могут вставлять узлы/фрагменты текста.

Итак, альтернативный вариант показать сообщение:
...
Мы могли бы создать функцию и положить код туда. Но альтернативным способом было бы *клонировать* существующий `div` и изменить текст внутри него (при необходимости).
...
`DocumentFragment` является специальным DOM узлом, который служит обёрткой для передачи групп узлов.

Мы можем добавить к нему другие узлы, но когда мы вставляем его куда-то, он "исчезает", оставляя вместо себя контент.
...
Чтобы удалить узлы, есть следующие методы:
...
## Слово о "document.write"

Это ещё один, очень древний метод добавления элементов на веб-страницу: `document.write`.
...
Вызов `document.write(html)` записывает `html` на страницу "прямо здесь и сейчас". `html`-строка может быть динамически сгенерирована, поэтому метод достаточно гибкий. Мы можем использовать JavaScript, чтобы создать полноценную веб-страницу и записать её.
...
Так что на этапе «после загрузки» он непригоден, в отличие от других методов DOM, которые мы рассмотрели выше.

Это было недостатком.

Технически, когда `document.write` запускается, пока браузер всё ещё читает HTML, метод добавляет что-то к HTML, и браузер использует его так же, будто это было изначально там.

Это даёт нам преимущество -- оно работает невероятно быстро, потому что *нет модификации DOM*. Метод пишет прямо в текст страницы, пока DOM ещё не создан, и браузер помещает его в DOM во время генерации.

Итак, если нам нужно динамически добавить много текста в HTML, и мы находимся на стадии загрузки, и для нас очень важна скорость, это может помочь. Но на практике эти требования редко сочетаются. И обычно мы можем увидеть этот метод в скриптах только потому, что он старый.
...
- Задан HTML-фрагмент: `elem.insertAdjacentHTML(where, html)`, вставляет в зависимости от `where`:
...
- Чтобы добавить HTML на страницу перед завершением её загрузки:
  - `document.write(html)`

  После загрузки страница такой вызов затирает документ. В основном встречается в старых скриптах.
