...
|---------|---------|---------|
| `async` | *Порядок загрузки*. Порядок скриптов в документе не играет роли - кто загрузится первым, тот и сработает |  Не имеет значения. Может загрузиться и выполниться до того, как страница полностью загрузится. Такое случается, если скрипты маленькие или хранятся в кеше, а документ достаточно большой. |
