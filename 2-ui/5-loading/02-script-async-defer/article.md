...
...Но `defer` не только говорит браузеру "не блокировать рендеринг", так же он обеспечивает последовательность выполнения скриптов. Даже если `small.js` загрузится первым, он будет ждать выполнения `long.js`.

Это может быть важно для случаев когда нам сначала нужно загрузить JavaScript библиотеку, от которой зависит наш скрипт.
```
