...
2. Shadow tree -- скрытое DOM-поддерево, не отражённое в HTML, скрытое от посторонних глаз.
...
Теневой DOM, если имеется, отрисовывается браузером вместо так называемого "light DOM" (обычные потомки). В главе <info:slots-composition> мы разберём, делать их композицию.
