...
Теневой DOM может содержать теги `<style>` и `<link rel="stylesheet" href="…">`. В последнем случае таблицы стилей кешируются по протоколу HTTP, так что они не будут загружаться повторно. Поэтому нет необходимости в использовании @import или добавлении ссылок на одни и те же стили для многих компонентов.
