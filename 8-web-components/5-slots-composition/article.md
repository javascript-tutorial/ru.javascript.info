...
Мы создали теневой DOM, он изображён под #shadow-root. Теперь у элемента есть два DOM-дерева: обычное и теневое.
...
Так что развёрнутый DOM составляется из теневого вставкой в слоты. Браузер использует его для рендеринга и при всплытии событий (об этом дальше). Но JavaScript видит документ "как есть" - до построения развёрнутого DOM-дерева.
...
Всё содержимое обычного дерева, не добавленное в слоты, попадает в `<fieldset>` "Другая информация".
