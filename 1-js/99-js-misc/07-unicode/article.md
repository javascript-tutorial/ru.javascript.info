...
- `\u{X…XXXXXX}`
    ...
    Вместо `X…XXXXXX` должно быть шестнадцатеричное число длиной от 1 до 6 цифр в диапазоне от `0` до `10FFFF` (максимальная точка кода, определённая стандартом Юникод). Эта нотация позволяет нам легко представлять все существующие символы Юникода.
