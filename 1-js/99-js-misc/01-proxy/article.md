...
| `[[GetOwnProperty]]` | `getOwnPropertyDescriptor` | [Object.getOwnPropertyDescriptor](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/getOwnPropertyDescriptor), `for..in` |
| `[[OwnPropertyKeys]]` | `ownKeys` | [Object.keys](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/keys), [Object.getOwnPropertyNames](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/getOwnPropertyNames), [Object.getOwnPropertySymbols](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/getOwnPropertySymbols), `for..in` |
...
- Метод `[[GetPrototypeOf]]`, применённый к прокси, должен возвращать то же значение, что и метод `[[GetPrototypeOf]]`, применённый к оригинальному объекту. Другими словами, чтение прототипа объекта `proxy` всегда должно возвращать прототип оригинального объекта.
...
Ловушка `set` должна вернуть `true`, если запись прошла успешно, и `false` в противном случае (ведёт к ошибке `TypeError`).
...
Нам не требуется перезаписывать методы массива `push` и `unshift` и другие, чтобы добавлять туда проверку на тип, так как внутри себя они используют операцию `[[Set]]`, которая перехватывается прокси.
...
sayHi("Вася"); // Hello, Вася! (через 3 секунды)
*/!*
```
...
В частности, `Reflect` позволяет вызвать операторы (`new`, `delete`...) как функции. Это интересная возможность, но здесь нам важно другое.

Для каждого внутреннего метода, перехватываемого `Proxy`, есть соответствующий метод в `Reflect`, который имеет такое же имя и те же аргументы, что и у ловушки `Proxy`. Поэтому мы можем использовать `Reflect`, чтобы перенаправить операцию на исходный объект.
...
В чём дело? Может быть, мы что-то не так с наследованием?

Но если убрать прокси (удалить строки 8-12), то всё будет хорошо.
...
3. При чтении из прокси свойства `name` срабатывает ловушка `get` и возвращает его из исходного объекта как `target[prop]` в строке `(*)`.
    ...
    Вызов `target[prop]`, если `prop` - это геттер, запускает его в контексте `this=target`. Поэтому результатом является `this._name` из исходного объекта `target`, то есть из `user`.
...
Можно переписать ловушку и кроче:
...
Методы в `Reflect` имеют те же названия, что и соответствующие ловушки, и принимают такие же аргументы. Это было специально задумано при добавлении `Reflect` и `Proxy` в язык JavaScript.
