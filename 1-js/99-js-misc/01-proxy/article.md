...
Для того, чтобы решить поставленную задачу, мы можем использовать отключаемый прокси, без всяких ловушек. Такой прокси будет передавать все операции на проксируемый объект, и у нас будет возможность отменить это поведение с помощью специального метода.
...
Вызов возвращает объект с `proxy` и функцией `revoke`, чтобы отключать действие прокси.
