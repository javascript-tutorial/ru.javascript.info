...
Для того, чтобы решить поставленную задачу, мы можем использовать отменяемый прокси, без всяких ловушек. Такой прокси будет передавать все операции на проксируемый объект, и у нас будет возможность отменить это поведение с помощью специального метода.
...
Вызов возвращает объект с `proxy` и функцией `revoke`, чтобы отменять действие прокси.
...
...Следует использовать прокси везде вместо оригинального объекта `target`. Прокси не имеет собственных свойств или методов. Он просто перехватывает операцию, если имеется соответствующая ловушка, или перенаправляет её сразу на объект `target`.
...
[Reflect](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Reflect) API создано как дополнение к [Прокси](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Proxy). Для любой ловушки из прокси осуществляется вызов соответствующего метода `Reflect` с теми же аргументами. Нам следует использовать их, чтобы перенаправить вызовы на оригинальный объект.
