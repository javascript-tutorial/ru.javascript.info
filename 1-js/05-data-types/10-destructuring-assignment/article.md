...
// Давайте поменяем местами значения: сделаем guest=Pete, admin=Jane
*!*
...
Здесь мы создаем временный массив из двух переменных и немедленно деструктурируем его в порядке замены.
...
alert(surname); // Anonymous (взялось значение по умолчанию)
```
...
alert(name);    // Julius (из массива)
alert(surname); // что угодно что получает prompt
```
