...
В приведённом ниже коде `options` хранит другой объект в свойстве `size` и массив в свойстве `items`. Шаблон в левой части присваивания имеет соответствующую структуру, чтобы извлечь данные из них:
...
Весь объект `options`, кроме свойства `extra`, которое никак не упоминалось, присваивается в соответствующие переменные:
