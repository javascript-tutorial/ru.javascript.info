...
Одинарные и двойные кавычки используются в языке с незапамятных времён: тогда потребность в многострочных строках не учитывалась. Что касается обратных кавычек, они появились существенно позже, и поэтому они гибче.
...
Разница только в том, что в случае отсутствия символа с такой позицией, тогда `[]` вернёт `undefined`, а `charAt` — пустую строку:
...
Если вы не планируете их поддерживать, этот раздел можно пропустить.
```
