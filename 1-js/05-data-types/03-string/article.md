...
|`\xXX`|Символ, код которого в стандарте [Юникод](https://ru.wikipedia.org/wiki/Юникод) равен `U+00XX`. `XX` - это ровно две шестнадцатеричные цифры со значением от `00` до `FF`, поэтому запись `\xXX` может использоваться только для первых 256 символов Юникода (включая все 128 символов [ASCII](https://ru.wikipedia.org/wiki/ASCII)). Например, `'\x7A'` — то же самое, что `'z'` (Юникодный код `U+007A`).|
|`\uXXXX`|Символ, код которого в стандарте Юникод равен `U+XXXX` (символ в кодировке UTF-16 с шестнадцатеричным кодом `XXXX`). `XXXX` - это ровно 4 шестнадцатеричные цифры со значением от `0000` до `FFFF`, поэтому запись `\uXXXX` может использоваться для первых 65536 символов Юникода. Символы с кодом большим, чем `U+FFFF`, также могут быть представлены с помощью данной записи, однако для этого нам потребуется использовать так называемые суррогатные пары (мы рассмотрим их позже в этой главе). Например, `"\u00A9"` - это символ копирайта `©` (его код в Юникоде `U+00A9`), а для символа улыбающегося кота 😺 приходится использовать суррогатную пару `"\uD83D\uDE3A"` (так как его код в Юникоде `U+1F63A` и он больше, чем `U+FFFF`).|
|`\u{X…XXXXXX}` (от 1 до 6 шестнадцатеричных цифр)|Символ с любым существующим кодом в стандарте Юникод (символ с заданным шестнадцатеричным кодом в кодировке UTF-32). `X…XXXXXX` - шестнадцатеричное число в диапазоне от `0` до `10FFFF` (самый большой код символа в Юникоде). Эта форма записи была добавлена в JavaScript в стандарте ECMAScript 2015 (ES6) и позволяет отображать любой символ Юникода без использования суррогатных пар. В отличие от двух предыдущих форм записи для символов Юникода, здесь нет обязательной необходимости добавлять нули перед шестнадцатеричным кодом символа с "маленьким" значением: варианты `"\u{7A}"`, `"\u{007A}"` и `"\u{00007A}"` одинаково допустимы.|

Примеры с Юникодом:

```js run
alert( "\u00A9" ); // ©, тот же самый результат дадут alert( "\xA9" ) и alert( "\u{A9}" )
...
: Возвращает часть строки *между* `start` и `end` (не включая большее из этих значений).
...
| `substring(start, end)` | между `start` и `end` (не включая большее значение)| отрицательные значения равнозначны `0` |
...
`str.codePointAt(pos)`
: Возвращает десятичное число, соответствующее коду символа, находящегося на позиции `pos`:
    ...
    alert( "z".codePointAt(0).toString(16) ); // 7a (если нам нужно получить шестнадцатеричное значение кода символа)
    ```
...
: Создаёт символ по его коду `code`
    ...
    alert( String.fromCodePoint(0x5a) ); // Z (мы можем использовать и шестнадцатеричное значение кода в качестве аргумента)
    ```
...
// codePointAt поддерживает суррогатные пары, но с некоторыми тонкостями

alert( '𝒳'.codePointAt(0).toString(16) ); // 1d4b3, считывает обе части суррогатной пары и возвращает корректный код символа 𝒳
alert( '𝒳'.codePointAt(1).toString(16) ); // dcb3, возвращает лишь вторую часть суррогатной пары
```
...
Например, буква `a` — это основа для `àáâäãåā`. Наиболее используемые составные символы имеют свой собственный код в таблице Юникод. Но не все, в силу большого количества комбинаций.

Чтобы поддерживать любые комбинации, стандарт Юникод позволяет использовать несколько Юникодных символов: основной и дальше один или несколько особых символов-знаков.
...
Разумеется, так происходит не всегда. Просто Ṩ — это достаточно часто используемый символ, поэтому создатели Юникода включили его в основную таблицу и присвоили ему код.
