...
Многострочные строки также можно создавать с помощью одинарных кавычек, используя так называемый "символ перевода строки", который записывается как `\n`:
...
|`\uXXXX`|Символ в кодировке UTF-16 с шестнадцатеричным кодом `NNNN`, например, `\u00A9` — юникодное представление знака копирайта, `©`. Код должен состоять ровно из 4 шестнадцатеричных цифр. |
...
Все спецсимволы начинаются с обратного слеша, `\`. Он также называется "символом экранирования".
...
Существует старый фокус с использованием [побитового оператора НЕ](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/Bitwise_Operators#.7E_.28Bitwise_NOT.29) — `~`. Он преобразует число в 32-разрядное целое со знаком — signed 32-bit integer. Десятичная дробь, в случае, если она присутствует, отбрасывается. Все биты числа инвертируются.
...
На данный момент такой фокус можно встретить в старом коде, а в новом он просто не нужен: есть метод `.includes` (см. ниже).
...
У этого метода есть два дополнительных аргумента, которые указаны в [документации](mdn:js/String/localeCompare). Первый позволяет указать язык (по умолчанию берётся из окружения) — от него зависит порядок букв. Второй — определить дополнительные правила, такие как чувствительность к регистру, а также следует ли считать эквивалентными `"a"` и `"á"`.
...
Дальше в главе <info:iterable> будет подробнее рассказано про суррогатные пары. Для работы с ними существуют специальные библиотеки, но нет ни одной достаточно широко известной, чтобы предложить здесь.
...
Это даёт большую гибкость, но из-за того, что порядок дополнительных символов может быть различным, становится возможным представить по-разному визуально ничем не отличающиеся символы.
...
Однако существует алгоритм "юникодной нормализации", приводящий каждую строку к единому "нормальному" виду.
