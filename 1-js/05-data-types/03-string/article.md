...
На данный момент такой трюк можно встретить в старом коде, а в новом он просто не нужен: есть метод `.includes` (см. ниже).
...
- Отрицательное число, если `str` меньше `str2`, т. е. `str` идёт до `str2`
- Положительное число, если `str` больше `str2`, т. е. `str` идёт после `str2`
