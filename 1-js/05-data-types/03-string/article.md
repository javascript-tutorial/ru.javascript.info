...
В частности, здесь мы два раза получим одинаковое сообщение:
...
Здесь перед входящей внутрь строки кавычкой необходимо добавить обратный слеш — `\'` — иначе она бы обозначала окончание строки.

Разумеется, это относится только к кавычкам, соответствующим тем, в которые заключена строка. Так что мы можем применить и более элегантное решение, использовав для этой строки двойные кавычки или обратные слеши:

```js run
alert( `I'm the Walrus!` ); // I'm the Walrus!
```

Надо отметить, что обратный слеш служит лишь для корректного прочтения строки интерпретатором, но он не записывается в строку после её прочтения. Когда строка сохраняется в оперативную память, в неё не добавляется символ `\`. Это можно легко продемонстрировать с помощью `alert` в примерах выше.

Но что, если нам надо добавить в строку обратный слеш?
...
Это очень удобно, но надо помнить и об интересной проблеме: два визуально ничем не отличающихся символа могут быть представлены по-разному.
...
Также есть методы для поиска и замены с использованием регулярных выражений. Но эта тема достаточно большая и достойна отдельной главы, поэтому мы вернёмся к ней позже.
