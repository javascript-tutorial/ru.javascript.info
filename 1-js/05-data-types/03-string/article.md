...
В JavaScript есть 3 метода для получения подстроки: `slice`, `substring` и `substr`.
...
: Как и `slice`, возвращает часть строки от `start` до (не включая) `end`, но можно задавать `start` больше `end`.
...
|--------|-----------|-----------|
| `slice(start, end)` | от `start` до (не включая) `end` | можно передавать отрицательные значения |
| `substring(start, end)` | от `start` до (не включая) `end` | отрицательные значения равнозначны `0` |
