...
JavaScript позволяет нам работать с примитивами (строками, числами и т.д.) так, как если бы они были объектами. Они также предоставляют методы для их вызова.  Мы скоро изучим их, но сначала разберём, как это всё работает, потому что, конечно, примитивы - не объекты (и сейчас мы это увидим).
