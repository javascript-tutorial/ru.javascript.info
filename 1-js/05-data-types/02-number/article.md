...
Здесь подчёркивание `_` играет роль "[синтаксического сахара](https://ru.wikipedia.org/wiki/%D0%A1%D0%B8%D0%BD%D1%82%D0%B0%D0%BA%D1%81%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B9_%D1%81%D0%B0%D1%85%D0%B0%D1%80)", оно просто делает число более читаемым. Движок JavaScript просто игнорирует `_` между цифрами, так что это точно такой же миллиард, как и выше.

Тем не менее в реальной жизни мы стараемся не писать длинные последовательности нулей. Мы слишком ленивы для этого. Мы стараемся писать что-то вроде `"1млрд"` для миллиарда или `"7.3млрд"` для 7 миллиардов 300 миллионов. То же самое верно и для большинства больших чисел.
...
Как и раньше, использование `"е"` может помочь. Если мы хотели бы избежать явного написания нулей, мы могли бы записать то же самое как:
...
Обратите внимание, что две точки в `123456..toString(36)` - это не опечатка. Если мы хотим вызвать метод непосредственно на числе, как `toString` в примере выше, то нам нужно поставить две точки `..` после него.
...
`Math.round`
: Округление до ближайшего целого: `3.1` становится `3`, `3.6` — `4`, среднее значение `3.5` тоже округлится до `4`.
...
2. Метод [toFixed(n)](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Number/toFixed) округляет число до `n-ого` количества цифр после точки и возвращает строковое представление результата.
    ...
    Мы можем преобразовать полученное значение в число, используя унарный оператор `+` или `Number()`, например написать `+num.toFixed(5)`.
...
Если число и в правду очень большое, оно может переполнить 64-битное хранилище, и превратиться в специальное числовое значение "бесконечность" `Infinity`:
...
Посмотрите на это (ложное!) сравнение:
...
Oй! Представьте, что вы создаёте интернет-магазин, и посетитель кладёт в свою корзину товары стоимостью `$0.10` и `$0.20`. Общая сумма заказа составит `$0.30000000000000004`. Это точно удивило бы любого.

Но почему это происходит?

Число хранится в памяти в двоичном виде, как последовательность битов - единиц и нулей. Но дроби, такие как `0.1`, `0.2`, которые выглядят довольно просто в десятичной системе счисления, на самом деле являются бесконечной дробью в двоичной форме.

Что такое `0.1`? Это единица, делённая на десять `1/10`, одна десятая. В десятичной системе счисления такое значение легко записать. А вот сравните с одной третьей `1/3`. Она станет бесконечной дробью `0.33333(3)`.

Таким образом, деление на степень `10` гарантированно хорошо работает в десятичной системе счисления, но деление на `3` - нет. По той же причине в двоичной системе счисления гарантированно работает деление на степень `2`, но `1/10` становится бесконечной двоичной дробью.
...
Мы также можем временно умножить числа на 100 (или даже на большее число), чтобы превратить их в целые числа, выполнить математические действия, а затем разделить обратно. Тогда когда мы совершаем математические операции с целыми числами, ошибка несколько уменьшается, но мы все равно получаем её при делении:
...
Иногда можно попытаться вообще избежать дробей. Например, если мы в нашем интернет-магазине начнём использовать центы вместо долларов. Но что будет, если мы применим скидку в размере 30%? На практике далеко не всегда можно полностью избавиться от дробей. Просто используйте округление, чтобы отрезать "хвосты", когда надо.
...
JavaScript не выдаст ошибку в таких случаях. Он делает все возможное, чтобы вписать число в нужный формат, но, к сожалению, этот формат недостаточно велик.
````
...
Помните эти особенные числовые значения?

- `Infinity` (и `-Infinity`) - это особое числовое значение, которое больше/меньше чем что-либо.
...
- `isNaN(value)` преобразует свой аргумент в число, а затем проверяет является ли оно `NaN`:
    ...
    Но нужна ли нам эта функция? Разве не можем ли мы просто использовать сравнение `=== NaN`? Извините, но мой ответ - нет. Значение `NaN` уникально тем, что оно не равно ничему, даже самому себе:
...
Они "считывают" число из строки до тех пор, когда не смогут этого продолжать. В случае ошибки возвращается собранное число. Функция `parseInt` возвращает целое число, в то время как `parseFloat` вернёт число с плавающей точкой:
...
- Добавьте "e" с указанным количеством нулей к числу. Например: `123e6` это `123` с 6-ю нулями `123000000`.
