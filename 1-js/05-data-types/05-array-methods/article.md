...
...Но если объект имеет специальное свойство `Symbol.isConcatSpreadable` и указать длину (количество свойств) объекта, то он обрабатывается `concat` как массив: вместо него добавляются его элементы:
