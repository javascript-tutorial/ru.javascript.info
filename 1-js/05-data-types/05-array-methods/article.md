...
...Но если объект, подобный массиву, имеет специальное свойство `Symbol.isConcatSpreadable`, то он обрабатывается как массив с помощью `concat`: вместо этого добавляются его элементы:
...
Например, давайте отсортируем несколько стран по-немецки:
...
alert( countries.sort( (a, b) => a.localeCompare(b) ) ); // Andorra,Österreich,Vietnam (верно!)
```
````
...
- [arr.some(fn)](mdn:js/Array/some)/[arr.every(fn)](mdn:js/Array/every) проверяет массив.
  ...
  Эти методы ведут себя примерно как операторы `||` и `&&`: если `fn` возвращает истинное значение, `arr.some()` немедленно возвращает `true` и останавливает перебор остальных элементов; если `fn` возвращает ложное значение, `arr.every()` немедленно возвращает `false` и также прекращает перебор остальных элементов.
