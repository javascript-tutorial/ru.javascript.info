...
Полученная строка `json` называется *JSON-форматированным*, *сериализованным*, *строчным* или же *структурированным* объектом. Мы можем отправить его по сети или поместить в обычное хранилище данных.
...
Обычно это нормально. Если это не то, что мы хотим, то скоро мы увидим, как можно адаптировать этот процесс.
...
Первый вызов - особенный. Ему передается специальный "объект-обертка": `{"": meetup}`. Другими словами, первая `(key, value)` пара имеет пустой ключ, а значением является целевой объект в целом. Вот почему первая строка из примера выше будет `":[object Object]"`.
...
Третий аргумент в `JSON.stringify(value, replacer, spaces)` - это количество отступов, используемых для удобного форматирования.

Ранее все строчные объекты не имели отступов и лишних пробелов. Это нормально, если мы хотим отправить объект по сети. Аргумент `spacer` используется исключительно для вывода в удобочитаемом виде.
...
Существует еще один формат [JSON5](http://json5.org/), который поддерживает ключи без кавычек, комментарии и т.д. Но это самостоятельная библиотека, а не спецификация языка.
