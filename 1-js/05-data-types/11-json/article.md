...
Полученная строка `json` называется *JSON-кодированным* или *сериализованным*, *строковым* или *структурированным* объектом. Мы готовы отправить его по сети или поместить в обычное хранилище данных.
...
// строка в JSON по-прежнему остается строкой, но в двойных кавычках
...
JSON является спецификацией только для данных на нескольких язках, поэтому некоторые специфичные для JavaScript свойства объекта в `JSON.stringify` пропускаются.
...
Первый вызов - особенный. Ему передается специальный "объект-обертка": `{"": meetup}`. Другими словами, первая `(key, value)` пара имеет пустой ключ, а значением является целевой объект в целом. Вот почему первая строка будет `":[object Object]"` в примере выше.
...
Давайте перейдем к функции восстановления `JSON.parse`, которая возвращает все значения «как есть», но `date` станет `Date`:
