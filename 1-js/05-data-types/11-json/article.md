...
alert(user); // {name: "Иван", age: 30}
```

...Но в процессе разработки добавляются новые свойства, старые свойства переименовываются и удаляются. Обновление такого `toString` каждый раз может стать проблемой. Мы могли бы попытаться перебрать свойства в нем, но что если объект сложный и имеет вложенные объекты в свойствах? Мы должны были бы также осуществить их преобразование. И, если мы отправляем объект по сети, нам также нужно предоставить код для "чтения" нашего объекта на принимающей стороне.
...
Полученная `json` строка называется *JSON-кодированным* или *сериализованным*, *строчным* или *структурированным* объектом. Мы готовы отправить его по сети или поместить в хранилище простых данных.


Обратите внимание, что объект в формате JSON имеет несколько важных отличий от объектного литерала:

- Строки используют двойные кавычки. Никаких одинарных кавычек или обратных кавычек в JSON. Так `'Иван'` становится `"Иван"`.
...
```js run
// число в JSON остается числом
alert( JSON.stringify(1) ) // 1

// строка в JSON остается строкой, но в двойных кавычках
...
JSON является спецификацией для данных на нескольких языках, поэтому некоторые специфичные для JavaScript свойства объекта в `JSON.stringify` пропускаются.
...
{
  "title":"Совещание",
  "room":{"number":23,"participants":["иванов","петров"]},
}
*/
```
...
В большинстве случаев `JSON.stringify` используется только с первым аргументом. Но если нам нужно не отфильтровать, а настроить процесс замены циклических ссылок, то можно использовать второй аргумент `JSON.stringify`.
...
let meetup = {
  title: "Совещание",
  participants: [{name: "Иван"}, {name: "Алиса"}],
...
// {"title":"Совещание","participants":[{},{}]}
```

Здесь мы, наверное, слишком строги. Список свойств применяется ко всей структуре объекта. так что свойство `participants` пустое, потому что `name` нет в списке.
...
Первый вызов - особенный. Ему передается специальная "объект-обертка": `{"": meetup}`. Другими словами, первая пара `(key, value)` имеет пустой ключ, а значением является целевой объект в общем. Вот почему первая строка будет `":[object Object]"` в примере выше.
...
Ранее все строковые объекты не имели отступов и лишних пробелов. Это нормально, если мы хотим отправить объект по сети. Аргумент `spacer` используется исключительно для вывода в удобочитаемом виде.
...
Параметр `spaces` используется исключительно для логирования и красивого вывода.
...
```js run
let user = '{ "name": "Иван", "age": 35, "isAdmin": false, "friends": [0,1,2,3] }';
...
```js
let json = `{
  *!*name*/!*: "Иванов",                     // Ошибка: имя свойства без кавычек
  "surname": *!*'Семенов'*/!*,               // Ошибка: одинарные кавычки в значении (должны быть двойными)
...
let str = '{"title":"Совещание","date":"2017-11-30T12:00:00.000Z"}';
```

...А теперь нам нужно *десериализовать* его, то есть снова превратить в объект JavaScript.
...
let schedule = `{
  "meetups": [
    {"title":"Совещание","date":"2017-11-30T12:00:00.000Z"},
    {"title":"День рождения","date":"2017-04-18T12:00:00.000Z"}
  ]
}`;
