...
Здесь мы, наверное, слишком строги. Список свойств применяется ко всей структуре объекта. Так что свойство внутри `participants`  -- пустые объекты, потому что `name` нет в списке.
...
Как видите, `toJSON` используется как при прямом вызове `JSON.stringify(room)`, так и если `room` вложен в другой сериализуемый объект.
