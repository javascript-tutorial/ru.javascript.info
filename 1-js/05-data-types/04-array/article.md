...
Некоторые языки программирования позволяют использовать отрицательные индексы для той же цели, например `fruits[-1]`.

Тем не менее в JavaScript это не сработает. Результат будет `undefined`.

Мы можем явно вычислить индекс последнего элемента, а затем получить к нему доступ, используя `fruits[fruits.length - 1]`:
