...
4. `Promise.any(promises)` -- ожидает первый успешно выполненный промис, который становится его результатом. В случае если ни один из промисов не завершится успешно, возвращает [`AggregateError`](mdn:js/AggregateError), содержащий причины отклонения всех промисов в свойстве `errors`.
