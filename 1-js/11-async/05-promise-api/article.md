...
```warn header="Новая возможность"
Эта возможность была добавлена в язык недавно. В старых браузерах может понадобиться [полифил](https://www.npmjs.com/package/promise-any-polyfill).

Обратите внимание что `Promise.any` в данный момент является эксперементальным, хоть и поддерживается большинством современных браузеров. В данный момент находится на стадии [TC39 Candidate stage (Stage 3)](https://github.com/tc39/proposal-promise-any).
```

Метод похож на `Promise.race`, возвращает первый **успешно выполненный(resolved)** промис, или массив содержащий причины отклонения всех промисов, если ни один из промисов не завершится успешно.
...
Например, тут результат будет `3`:
...
Быстрее всех выполнился второй промис, но так как `Promise.any` ждёт первый **успешно выполненный** промис, то результат мы получим из третьего. Первый промис будет проигнорирован.
...
4. `Promise.any(promises)` -- ожидает первый **успешно выполненный(resolved)** промис, который становится его результатом, остальные игнорируются. В случае если ни один из промисов не завершится успешно, возвращает массив, содержащий причины отклонения всех промисов.
