...
Поэтому макрозадачи выполняются, только когда промисы дают движку "свободное время". Так, если у нас есть цепочка обработчиков промисов, которые выполняются один-за-другим, без задержек, то `setTimeout` или обработчик действия пользователя не может попасть в середину, между ними.
...
Снова появилось событие о необработанной ошибке. Почему? Потому что `unhandledrejection` генерируется, когда очередь микрозадач выполнена. Движок проверяет промисы и, если если какой-либо из них в состоянии "rejected", то генерируется это событие.
