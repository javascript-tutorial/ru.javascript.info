...
Такие функцию называют «асинхронными», потому что действие (загрузка скрипта) будет выполнено не сейчас, а потом.

Вызов функции инициирует загрузку скрипта, после исполнение кода продолжается. Пока скрипт загружается, код, описанный ниже, может выполнится до конца. А если загрузка занимает много времени, могут быть запущены и другие скрипты.
...
Теперь, если мы хотим вызвать функцию из скрипта, нужно делать это в функции обратного вызова:
...
Такое написание называют асинхронным программированием с помощью функций обратного вызова. В функции, которые выполняют какие-либо асинхронные операции, передается аргумент `callback` — функция, которая будет вызвана по завершению асинхронного действия.

Мы поступили похожим образом в `loadScript`, но это, конечно, распространенный подход.

## Функция обратного вызова в функции обратного вызова

Как нам загрузить два скрипта один за другим: сначала первый, а за ним второй?

Первое, что приходит в голову, вызвать `loadScript` еще раз уже внутри функции обратного вызова, вот так:
...
Когда внешняя функция `loadScript` выполнится, вызовется та, что внутри функции обратного вызова.
...
Каждое новое действие мы вынуждены вызывать внутри функции обратного вызова. Этот вариант подойдет, когда у нас одно-два действия, но для большего количества — это уже не вариант. Что делать в этом случае, мы рассмотрим чуть позже.

## Перехват ошибок

В примерах выше мы не думали об ошибках. А что если наш скрипт не загрузится? Мы должны предусмотреть этот вариант в функции обратного вызова.

Вот улучшенная версия `loadScript`, которая умеет отслеживать ошибки загрузки:
...
Мы вызываем `callback(null, script)` в случае успешной загрузки и `callback(error)`, если загрузить сккрипт не удалось.
...
Опять же, подход, который мы использовали в `loadScript`, также распространен и называется «функция обратного вызова, где первый аргумент ошибка».
...
Иногда это называют «адом обратных вызовов» или «пирамидой ужаса».
...
Код абсолютно рабочий, но кажется разованным на куски. Его тружно читать, вы наверняка заметили это. Приходится прыгать глазами между кусками кода, когда пытаешься его прочесть. Это неудобно, особенно, если читатель не знаком с кодом и не знает, что за чем следует.

Кроме того, все функции `step*` одноразовые, и созданы лишь с той целью, чтобы избавиться от «пирамиды ужаса». Никто не будет их переиспользовать где-либо еще. Таким образом мы, кроме прочего, засоряем пространство имен.

Нужно найти способ получше.

К счастью, такие способы существуют. Один из лучших — использовать «обещания», о которых рассказано в следующей главе.
