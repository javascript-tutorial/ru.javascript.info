...
Если у нас нет `try..catch`, она функция будет возвращать завершившийся с ошибкой промис (в состоянии rejected). В этом случае мы можем использовать метод `.catch` промиса, чтобы обработать ошибку:
...
2. Иначе возвратит результат промиса.
