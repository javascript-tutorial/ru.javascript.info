...
- `result` ("результат") — имеет произвольное значение, изначально `undefined`.
...
Это может случится, например, когда мы начали выполнять какую-то задачу, но тут же увидели, что ранее ее уже выполняли, и результат закеширован.
...
2. `Finally` "пропускает" результат или ошибку к последующим обработчикам.
