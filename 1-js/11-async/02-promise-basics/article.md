...
В таком случае эти обработчики запускаются сразу после выполнения текущего кода (подробнее асинхронность обработчиков промисов рассмотрена в главе <info:microtask-queue>):
