...
Вызов `.finally(f)` это тоже самое, что передать в `.then(f, f)`, в том смысле, что "потребляющий" код выполнится в любом случае, когда промис станет решённым: успешно или с ошибкой.
