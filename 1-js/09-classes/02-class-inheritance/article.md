...
Но если мы укажем наш собственный метод в `Rabbit`, например `stop()`, то он будет использоваться взамен родительского:
...
Если вы нашли этот материал слишком сложным, просто продолжайте читать, а затем вернитесь к нему через некоторое время.
```
