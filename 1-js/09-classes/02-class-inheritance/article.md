...
*!*
  stop() {
    super.stop(); // вызываем рподительский метод stop
...
Коротко: конструктор в классах потомках должен вызывать `super(...)` и (!) делать это перед использованикм `this`.
...
Давайте попробуем это сделать. Без классов, используя простые объекты дял наглядности.
...
Каждый методзапоминает его объект во внутреннем свойстве `[[HomeObject]]`. Затем `super` использует его для разрешения родительского прототипа.
