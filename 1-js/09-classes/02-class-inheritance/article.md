...
Давайте попробуем прикинуть, как технически это может работать. Когда метод объекта выполняется, он получает текущий объект как `this`. Если мы вызываем `super.method()`, то ему необходимо плучить `method` из прототипа текущего объекта. Как движок JavaScript должен получить прототип `this`?

Задача может показаться простой, но это не так. Движок может попытаться получить метод из `[[Prototype]]` `this`, используя `this.__proto__.method`. К сожалению, это не работает.
...
Это фактически нарушает идею «свободных» функций, потому что методы помнят свои объекты. `[[HomeObject]]` нельзя изменить, поэтому эта связь навсегда. Это очень важное изменение в языке.

Но это изменение в языке безопасно. `[[HomeObject]]` используется только для вызова родительских методов в `super` чтобы найти прототип.  Так что оно не сломает существующий код. Обычные вызовы ничего про `[[HomeObject]]` не знают, он только для super.
...
Каждый метод запоминает свой объект во внутреннем свойстве `[[HomeObject]]`. Затем `super` использует его для получения родительского прототипа.

`[[HomeObject]]` определен для методов и в классах и в литеральных объектах. Но для объектов методы должны быть определены именно как `method()`, а не как  `"method: function()"`.
