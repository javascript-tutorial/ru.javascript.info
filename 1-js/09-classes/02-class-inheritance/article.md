...
Но мы хотим, чтобы `Rabbit` расширял `Animal`. Другими словами, кролики должны исходить из животных, т.е. они имеют доступ к методам `Animal` и расширять его функциональность своими методами.
