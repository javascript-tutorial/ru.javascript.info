...
Итак, что же такое `class`? Нам может казаться, что это новая сущность на уровене языка, но это не так. 
...
Как и функции, классы погут быть определены внутри другого выражения, переданы, возвращены и т.д. 
