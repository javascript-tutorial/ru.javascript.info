...
Другими словами, *примесь* определяет методы, которые реализуют определённое поведение. Мы не используем примесь саму по себе, а используем `[comment]: не понятен смысл предложения, кажется здесь что-то пропущено (возможно - лишь её методы),`, чтобы добавить функционал другим классам.
