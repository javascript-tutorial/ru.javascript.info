...
Статические методы (static) используются для реализации функций(методов), которые будут принадлежать классу целиком, но при этом не относящимся к каким-то отдельным объектам.
