...
Если после создания свойство `F.prototype` изменится (`F.prototype = <другой объект>`), то новые объекты, созданные `new F`, будут иметь в качестве [[Prototype]] другой объект, а уже существующие объекты сохранят старый.
```
