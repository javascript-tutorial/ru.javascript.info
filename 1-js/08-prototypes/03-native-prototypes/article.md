...
```warn header="Значения `null` и `undefined` имеют объектов-обёрток"
...
Полифил - это термин, который означает замену метода, существуего в спецификации JavaScript, но ещё не поддерживается текущим движком JavaScript.
