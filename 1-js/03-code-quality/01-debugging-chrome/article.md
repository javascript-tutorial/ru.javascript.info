...
<span class="devtools" style="background-position:-72px -76px"></span> – сделать шаг в следующую функцию. Быстрая клавиша – `key:F11`.
: При нажатии на эту клавишу, мы будем продвигаться шаг за шагом по коду до ближайшей функции, в которую провалимся и также полностью по ней пройдем. При этом вызов данной функции может быть как синхронный (обычный вызов функции), так и асинхронный (вызов функции через таймауты/интервалы, после ответа на сетевые запросы). В случае асинхронного вызова, дебаггер дождется, когда будет вызвана функция и провалится в нее.
