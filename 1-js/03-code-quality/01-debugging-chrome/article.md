...
Панель Sources разделена на 3 части:

1. **File Navigator** содержит файлы HTML, JavaScript, CSS, включая изображения, используемые на странице. Здесь также могут быть файлы различных расширений Chrome.
2. **Code Editor** показывает исходный код.
3. **JavaScript Debugging** отведена для отладки, вскоре мы к ней вернёмся.

Чтобы скрыть список файлов и освободить экранное место для исходного кода, щёлкните по тому же переключателю <span class="devtools" style="background-position:-172px -122px"></span>.
...
: Выполнить следующую инструкцию. Если кликнуть на неё, то вызовется `alert`.

    Каждое последующее нажатие будет выполнять наш код шаг за шагом

<span class="devtools" style="background-position:-62px -192px"></span> – "Step over": выполнить следующую команду, *не заходя в функцию*. Быстрая клавиша – `key:F10`.
: То же, что и "Step", но работает по другому в том случае, если следующая инструкция -- это вызов функции. Но не встроенной как `alert`, а нашей собственной.

    Если "Step" позволяет зайти в функцию и приостановить выполнение скрипта на её первой строке, то "Step over" выполняет вызов функции, пропуская её внутреннюю реализацию.

    В таком случае выполнение кода приостанавливается на строке после функции.

    Это удобно, если нас не интересует то, что происходит внутри неё.

<span class="devtools" style="background-position:-4px -194px"></span> – "Step into". Быстрая клавиша – `key:F11`.
: То же, что и "Step", но работает по другому в случае вызова асинхронных функций. Если вы только начали изучение JavaScript, то можете просто проигнорировать эту разницу -- пока у нас нет таких вызовов.

    Запомните на будущее, что "Step" пропускает асинхронные действия, такие как `setTimeout` (отложенный вызов функции), которые могут быть выполнены позже. "Step into" заходит в код таких функций и ожидает их завершения, если это необходимо. Можно ознакомиться с [Руководством по DevTools](https://developers.google.com/web/updates/2018/01/devtools#async) для получения дополнительной информации.
...
: Продолжает выполнение кода и приостанавливает его на самой последней строчке текущей функции. Этот метод применяется, когда мы случайно нажали <span class="devtools" style="background-position:-200px -190px"></span> и зашли в функцию, но нам она неинтересна и мы как можно скорее хотим из неё выбраться.
...
Обычный пользователь сайта не увидит такой вывод, так как он в консоли. Напомним, что консоль можно открыть через инструменты разработчика – выберите панель "Console" или нажмите `key:Esc`, находясь на другой панели – консоль откроется в нижней части интерфейса.
