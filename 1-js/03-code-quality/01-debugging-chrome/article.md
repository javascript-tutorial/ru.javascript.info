...
– сделать шаг в обычном (синхронном) порядке исполнения кода. Быстрая клавиша – `key:F9`.
: Код будет выполняться шаг за шагом, с прохождением каждой строчки кода и с заходом в каждую функцию, за исключением асинхронных вызовов (таймауты/интервалы, сетевые запросы). В них дебаггер заходить не будет.
