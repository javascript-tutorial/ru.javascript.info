...
Например, мы работаем наз функцией `f`. Написали часть кода и решили протестировать. Выясняется что `f(1)` рабоает правильно а `f(2)` нет. Мы вносим в код исправления и теперь `f(2)` работает правильно. Вроде бы все хорошо, так? Однако мы забыли заного протестировать `f(1)`. Возможно после внесения правок `f(1)` стала работать неправильно.  

Это типичная ситуация. Во время разработки мы держим в голове множество различных ситуаций. Но сложно ожидать что программист станет вручную проверять каждую из них после каждого изменения кода.

**Автоматическое тестирование означает что тесты пишутся отдельно, в дополнение к коду. Их можно легко запускать и они проверяют все основные способы использования.**
...
**BDD сочетает в себе три вещи: тесты, документация и примеры использования**
