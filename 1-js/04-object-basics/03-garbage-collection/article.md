...
Основной алгоритм сборки мусора называется "разметка и зачистка" (от англ. "mark-and-sweep").
...
ТТеперь объекты, которые не удалось посетить в процессе, считаются недостижимыми и будут удалены:
