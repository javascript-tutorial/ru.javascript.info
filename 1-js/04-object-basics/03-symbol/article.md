...
Он проверяет глобальный реестр, если в нём есть символ с именем `key`, то возвращает его, иначе создаёт новый символ `Symbol(key)` и записывает его в реестр под ключом `key`.
