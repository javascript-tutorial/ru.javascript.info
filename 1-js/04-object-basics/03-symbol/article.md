...
Представьте, что другой скрипт для каких-то целей хочет записать собственное свойство "id" в объект `user`. Этот скрипт может быть другой JavaScript-библиотекой, абсолютно не связанной с вашим скриптом. 

Тогда тот скрипт может создать свой символ `Symbol("id")`:
