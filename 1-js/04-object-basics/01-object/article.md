...
Возможно определение объекта без явного указания ключа, указав переменные через запятую:

```js
let name = "John";
let age  = 30;
let user = { name, age }; // определение объекта аналогично { name: "John", age: 30 };  
