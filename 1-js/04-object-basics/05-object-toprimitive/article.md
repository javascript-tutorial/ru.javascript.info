...
2. Иначе, если хинт равен `"string"`
    - пытается вызвать `obj.toString()`, если его нет, то `obj.valueOf()`, если он существует.
3. В случае, если хинт равен `"number"` или `"default"`
    - пытается вызвать `obj.valueOf()`, если его нет, то `obj.toString()`, если он существует.

## Метод Symbol.toPrimitive

Начнем с универсального подхода - символа `Symbol.toPrimitive`, который можно использовать в качестве ключа для метода преобразования:
