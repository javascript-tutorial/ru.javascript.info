...
И, также как и в реальном мире, пользователь может *совершать действия*: выбирать что-то из корзины покупок, авторизоваться, выйти из системы, оплатить и т.п.
...
В JavaScript ключевое слово "this" ведет себя иначе чем в большинстве других языках программирования. Во-первых, оно может использоваться в любой функции.
...
Например, одна и та же функция может иметь различное значение "this" когда вызывается из разных объектов:
...
Пожалуйста, обратите внимание, что обычно вызов функции, которая использует `this`, без объекта - не является нормальным и больше говорит об ошибке в программировании. Если функция использует `this`, тогда это обычно означает, что она будет вызываться в контексте какого-либо объекта.
...
Не очевидный вызов метода может привести к потери значения `this`, например:
