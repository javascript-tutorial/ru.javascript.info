...
Может показаться, что строка `(*)` вызовет ошибку, но это не так. Значение `user` является постоянным, оно всегда должно ссылаться на один и тот же объект, но свойства этого объекта могут свободно изменяться.

Другими словами, `const user` выдает ошибку только в том случае, если мы пытаемся полностью переназначить `user=...`.

Тем не менее, если нам действительно нужно создать постоянные свойства объекта, это тоже возможно, но с использованием совершенно других методов. Мы затронем эту тему в главе <info:property-descriptors>.
````
