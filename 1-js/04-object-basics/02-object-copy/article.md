...
user.sizes.width = 60;       // изменяем свойства в первом объекте
alert(clone.sizes.width); // 60, видим результат в другом
```

Чтобы исправить это и сделать `user` и `clone` действительно отдельными объектами, мы должны использовать цикл клонирования, который проверяет каждое значение `user[key]` и, если это объект, затем реплицирует его структуру. Это называется "глубоким клонированием" или "структурированным клонированием". Существует метод [structuredClone](mdn:API/structuredClone), реализующий глубокое клонирование.

### structuredClone

Вызов `structuredClone(object)` клонирует `object` со всеми вложенными свойствами.
...
// user и clone теперь совершенно не связаны
user.sizes.width = 60;    // изменяем свойства в первом объекте
alert(clone.sizes.width); // 50, не видим результат в другом
```

Метод `structuredClone` может клонировать большинство типов данных, таких как объекты, массивы, примитивные значения.
...
```js run
let user = {};
// давайте создадим циклическую ссылку:
// user.me ссылается на самого себя (user)
user.me = user;
...
Как вы можете видеть, `clone.me ` ссылается на `clone`, а не на `user`! Таким образом, циклическая ссылка также была клонирована правильно.

Хотя бывают случаи, когда `structuredClone` терпит неудачу.

Например, когда объект имеет функцию в качестве свойства:
...
Свойства содержащие функции не поддерживаются.

Для решения таких сложных случаев нам может потребоваться использовать комбинацию методов клонирования, написать собственный код или, чтобы не изобретать велосипед, возьмите готовую реализацию, например [_.cloneDeep(obj)](https://lodash.com/docs#cloneDeep) из библиотеки JavaScript [lodash](https://lodash.com).
...
Чтобы создать "реальную копию" (клон), мы можем использовать `Object.assign` для так называемой "поверхностной копии" (вложенные объекты копируются по ссылке) или функцию "глубокого клонирования" `structuredClone` или можно использовать специальную реализацию клонирования, такую как [_.cloneDeep(obj)](https://lodash.com/docs#cloneDeep).
