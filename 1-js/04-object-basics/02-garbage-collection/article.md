...
Если перезаписать значение `user`, ссылка потеряется:
...
Функция `marry` "женит" два объекта, давая им ссылки друг на друга и возвращает новый объект, содержащий ссылки на оба объекта.
