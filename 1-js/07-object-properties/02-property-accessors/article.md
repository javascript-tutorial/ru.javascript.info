...
Ещё раз заметим, что свойство объекта может быть только свойством-аксессором (с методами `get/set`) или свойством-данным (со значением `value`).
...
Например, представим, что мы начали реализовывать объект `user`, используя свойства-данные `name` и `age`:
...
Теперь старый код тоже работает, а у нас есть отличное дополнительное свойство!
