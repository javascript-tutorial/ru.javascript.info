...
В этой главе мы изучим дополнительные опции конфигурации свойств, а в следующей увидим, как легко превратить их в специальные функции - геттеры и сеттеры.
...
Мы ещё не встречали эти атрибуты, потому что они имеют скрытую реализацию. Когда мы создаём свойство "обычным способом", все эти атрибуты имеют значение `true`. Но мы можем изменить их в любое время.
...
Если мы этого не хотим, можно установить для свойства `enumerable:false`. Тогда оно перестанет появляться в цикле `for..in`, как и его встроенный аналог:
...
В нестрогом режиме мы не увидим никаких ошибок при записи в свойства "только для чтения" и т.п. Эти операции всё равно не будут выполнены успешно. Действия, нарушающие ограничения флагов свойств в нестрогом режиме, просто молча игнорируются.
```
...
Другое отличие в том, что `for..in` игнорирует символы, а `Object.getOwnPropertyDescriptors` возвращает дескрипторы *всех* свойств, включая свойства-символы.
