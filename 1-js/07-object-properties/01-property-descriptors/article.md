...
В этой главе мы изучим дополнительные параметры конфигурации свойств, а в следующей увидим, как легко превратить их в специальные функции - геттеры и сеттеры.
...
Если мы этого не хотим, можно установить для свойства `enumerable:false`. Тогда перестанет появляться в цикле `for..in`, как и его встроенный аналог:
...
Определение свойства как неконфигурируемого - это дорога в один конец. Мы не сможем поменять его обратно, потому что `defineProperty` не работает с неконфигуриремыми свойствами.
...
В нестрогом режиме мы не увидим никаких ошибок при записи в свойства "только для чтения" и т.п. Эти операции все равно не будут выполнены успешно. Действия, нарушающие ограничения флагов свойств в нестрогом режиме, просто молча игнорируются.
```
