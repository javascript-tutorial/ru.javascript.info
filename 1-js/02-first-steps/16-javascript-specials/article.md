...
Сравнение
: Проверка на равенство `==` значений разных типов конвертирует их в число (за исключением `null` и `undefined`, которые равны друг другу и сами себе), так что примеры ниже равны:
    ...
    Значения `null` и `undefined` особенные: они равны `===` только сами себе, но не равны ничему ещё.
