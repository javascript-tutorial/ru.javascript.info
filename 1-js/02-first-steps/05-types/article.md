...
Чтобы было понятнее: `typeof` — это оператор, а не функция. Скобки здесь не являются частью `typeof`. Это скобки для математического группирования.

Обычно такие скобки содержат математическое выражение, например `(2 + 2)`, но здесь они содержат только один аргумент `(x)`. Синтаксически, они позволяют избежать пробела между оператором `typeof` и его аргументом, и некоторым людям это нравится.

Некоторые предпочитают `typeof(x)`, хотя синтаксис `typeof x` гораздо более распространён.
```
