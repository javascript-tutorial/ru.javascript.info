...
Чтобы сделать это, воспользуемся оператором `if` и условным (тернарным) оператором, который мы будем называть оператором “вопросительный знак” (`?`) для простоты.
...
Если мы хотим выполнить более одной команды, мы должны заключить наш блок кода в фигурные скобки:
...
Так называемый "тернарный" или оператор "вопросительный знак" позволяет нам сделать это более коротким и простым способом.

Оператор представлен знаком вопроса `?`. Формальный термин "тернарный" означает, что оператор имеет три аргумента. На самом деле это единственный оператор в JavaScript, который имеет так много аргументов.
...
При чтении, глаза сканируют код по вертикали. Кодовые блоки, занимающие несколько строк, воспринимаются гораздо легче, чем длинный горизонтальный набор инструкций.
