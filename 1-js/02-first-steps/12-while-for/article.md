...
Если бы `i++` отсутствовало в примере выше, то цикл бы повторялся (в теории) вечно. На практике же, браузер предоставляет способы останавливать такие циклы, а в JavaScript на стороне сервера, мы можем просто "убить" этот процесс.

Любое выражение или переменная может быть условием цикла, не только сравнения: условие `while` вычисляется и конвертируется в boolean.
...
Такая форма синтаксиса оправдана, если вы хотите, чтобы тело цикла выполнилось **хотя бы однажды** вне зависимости от того, является ли условие `true`. Чаще используется обычная форма: `while(…) {…}`.
...
Директива `continue` -- "облегчённая версия" `break`. Она прерывает не весь цикл, а лишь останавливает текушую итерацию и вынуждает цикл перейти к следующей(если позволяет условие).
...
Для чётных значений `i`, директива `continue` прекращает выполнение тела цикла и передаёт управление на следующую итерацию цикла `for` (со следующим числом). Таким образом `alert` вызывается только для нечётных значений.

````smart header="Директива `continue` избегать вложенности"
...
С технической точки зрения он полностью идентичен. Действительно, вместо continue можно просто завернуть действия в блок `if` вместо использования `continue`.
...
...и перепишем его использую вопросительный знак:
...
В примере выше это означает, что вызовом `break outer` будет разорван самый внешний цикл до метки с именем `outer` и управление перейдёт с `(*)` к `alert('Done!')`.

Также можно разместить метку на отдельной строке:
