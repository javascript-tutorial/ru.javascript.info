...
alert( 8 ** (1/3) ); // 2 (степень 1/3 равна взятию кубического корня)
```
...
Есть ли разница между ними? Да, но увидеть её мы сможем, только если будем значение, которое возвращают `++/--`.
