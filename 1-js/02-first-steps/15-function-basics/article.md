...
Если одноимённая переменная 

ется внутри функции, тогда она перекрывает внешнюю. Например, в коде ниже функция использует локальную переменную `userName`. Внешняя будет проигнорирована:
...
В нижеприведённом примере функция объявлена с двумя параметрами: `from` и `text`.
...
Когда функция вызывается в строках `(*)` и `(**)`, переданные значения (*аргументы*) копируются в локальные переменные `from` и `text`. Затем они используются в теле функции.
...
```smart header="Параметры и аргументы"
Мы объявляем функцию с *параметрами*;
Мы вызываем функцию с *аргументами*;
```
