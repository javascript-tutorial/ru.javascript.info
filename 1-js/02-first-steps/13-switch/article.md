...
Несколько вариантов `case`, использующих один код, можно сгруппировать.

Для примера, если мы захотим выполнить один и тот же код для `case 3` и `case 5`:
...
Нужно отметить, что проверка на равенство всегда строгая. Значения должны быть одного типа, чтобы выполнилось равенство.
...
1. Для `0`, `1`, выполнится первый `alert`.
2. Для `2` -- второй `alert`.
3. Но для `3`, результатом `prompt` будет строка `"3"`, которая не соответствует строгому равенству `===` числу `3`. Таким образом, мы имеем "мёртвый код" в `case 3`! Выполнится вариант `default`.
