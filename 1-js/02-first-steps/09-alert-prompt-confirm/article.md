...
Но мы продолжим использовать браузер как демо-среду, потому что нам нужно познакомиться по крайней мере с несколькими функциями его пользовательского интерфейса. В этой главе ознакомимся с функциями `alert`, `prompt` и `confirm`.
