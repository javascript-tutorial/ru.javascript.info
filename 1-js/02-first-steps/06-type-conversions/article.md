...
Чаще всего, операторы и функции автоматически приводят переданные им значения к нужному типу. Это и называется "преобразованием типов".
...
Преобразование происходит наиболее очевидным способом. `false` становитмя `"false"`, `null` становится `"null"`, и т.п.
...
Явноe преобразование часто применятся, когда мы ожидаем получить число из строковых источников, вроде форм текстового ввода.
...
Учтите, что `null` и `undefined` ведут себя по разном. Так, `null` становится нулём, когда `undefined` приводится к `NaN`.

````smart header="Сложение '+' конкатенация строк"
...
Так происходит, только если хотя бы один из агрументов является строкой. Во всех остальных случаях, значения складываются как числа.
````
...
Происходит в логических операторах  (позже мы познакомимся с условиями подобными констукциями) но так же может быть выполнено явно с помощью функции `Boolean(value)`.
...
Некоторые языки (а именно PHP) воспринимают `"0"` как `false`. Но в JavaScript, если строка не пустая, то она всегда `true`.

```js run
alert( Boolean("0") ); // true
alert( Boolean(" ") ); // пробел это тоже true (любая не пустая строка это true)
```
````
