...
Этот пример чётко демонстрирует одно из главных предназначений фунций: избавление от дублирования кода.
...
function showMessage() {
  *!*userName*/!* = "Bob"; // (1) изменяем значение внешней меременной
...
Вот ещё один пример: у нас есть переменная `from` и мы передаём её функции. Обратите внимание: функция изменяет значение `from`, но эти изменение не видны снаружи, потому что функция всегда получает только копию значения:
...
Это не приведёт к ошибке. Такой вызов выведет `"Ann: undefined"`. Здесь не указан параметр `text`, поэтому предполагается, что `text === undefined`.
...
Тут `"no text given"` это строка, но на ёё месте могло бы быть и более сложное выражение, которое бы вычислялось и присваивалось при отсутствии параметра. Например:
...
...Или с помощью логического оператора `||`:
...
Простейшим примером может послужить функция сложения двух чисел:
...
Эти примеры предполагают общие смыслы префиксов. Что они значат для вас определяете вы и команда разработчиков, частью которой вы являетесь. Может быть совершенно нормальным для вашего кода иметь в этом отличия. Но вы должны твёрдо понимать, что означают префиксы имён, что функция с ним может делать, а что нет. Все одно-префиксные функции должны подчиняться одним правилам. И все разработчики в команде должны знать об этом.
```
...
Разделение функций не только облегчает тестирование и отладку -- само существование такой функции, выполняет роль хорошего комментария!

Например, сравним две функции `showPrimes(n)` ниже. Каждая из них выводит [простое число](https://ru.wikipedia.org/wiki/%D0%9F%D1%80%D0%BE%D1%81%D1%82%D0%BE%D0%B5_%D1%87%D0%B8%D1%81%D0%BB%D0%BE) до `n`.
...
Второй вариант легче для понимания, не правда ли? Вместо куска кода мы видим название действия (`isPrime`). Иногда разработчики назвают такой код *самодокументируемым кодом*.
...
Для того, чтобы сделать код более чистым и лёгким для понимания, рекомендуется в основном использовать локальные переменные и параметры функций, не пользоваться внешними переменными.
