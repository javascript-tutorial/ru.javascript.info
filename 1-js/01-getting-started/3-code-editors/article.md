...
- Я бы использовал [Visual Studio Code](https://code.visualstudio.com/), если бы в основном писал фронтенд.
- Иначе, если это частично фронтенд, а основа — на другом языке или платформе, то стоит рассмотреть другие редакторы, такие как XCode (Mac), Visual Studio (Windows) или семейство Jetbrains (Webstorm, PHPStorm, RubyMine и т.д., в зависимости от языка).
