...
3. Метод `next()` не обязательно должен быть `async`, он может быть обычным методом, возвращающим промис, но `async` позволяет использовать `await`, так что это удобно. Здесь мы просто создаём паузу на одну секунду `(3)`.
4. Для итерации мы используем `for await (let value of range)` `(4)`, добавляя "await" после "for". Метод `range[Symbol.asyncIterator]()` вызовется один раз, а затем будет работать `next()` для получения значений.
...
Мы бы хотели, чтобы вызов `fetchCommits(repo)` получал для нас коммиты, делая запросы всякий раз, когда это необходимо. И пусть он сам разбирается со всем, что касается нумерации страниц, для нас это будет просто `for await..of`.
