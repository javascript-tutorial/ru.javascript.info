...
Например, когда мы загружаем что-то по частям или ожидаем, что события будут происходить асинхронно, и нам хотелось бы их перебирать - как раз и пригождаются асинхронные итераторы и генераторы. Давайте сначала рассмотрим простой пример, чтобы понять синтаксис, а затем - реальный практический.
...
Например, оператор spread (расширение) не будет работать:
```js
