...
Например, оператор spread не будет работать:
```js
...
Как раз то, что мы и хотели. Внутренняя механика по-страничной подгрузки данных не видна снаружи. Для нас это просто async-генератор, возвращающий коммиты.
