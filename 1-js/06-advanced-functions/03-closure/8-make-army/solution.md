...
Старый вариант решения с помощью IIFE:
...
Помещаем код в анонимную функцию и сразу же ее вызываем, передавая в нее `i`. Таким образом на каждой итерации `i` берется из лексического окружения этой функции.
