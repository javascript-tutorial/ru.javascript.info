...
JavaScript - язык, с сильным функционально-ориентированным уклоном. Это даёт нам много свободы. Функция может быть создана в один момент, а затем скопирована в другую переменную или передана как аргумент другой функции и позже вызывана из совершенно другого места.
...
В JavaScript у каждой выполняемой функции, блока кода и скрипта есть связанный с ними объект, называемый *Лексическое Окружение*.

Объект Лексического Окружения состоит из двух частей:

1. *Environment Record* (Данные Окружения) -- объект, у которого все локальные переменные являются его свойствами (и некоторая другая информация, такая как значение `this`).
...
**В отличии от `let` переменных, они полностью инициализируются не тогда, когда выполнение доходит до них, а раньше, когда создаётся Лексическое Окружение.**
...
В течении вызова `say()` использует внешнюю переменную `phrase`. Давайте разберемся подробно, что происходит.
...
Мы так же можем использовать "голые" блоки кода `{...}`, чтобы изолировать переменные в "локальной области видимости".
...
Так что программистам пришлось что-то придумать. И то, что они сделали, называется "immediately-invoked function expressions", то есть, функция, которая выполняется сразу же после того, как была определена (абревиатура IIFE).
...
Пожалуйста, обратите внимание, если `f()` вызывается несколько раз и возвращаемые функции сохраняются, тогда соответствующие объекты Лексического Окружения продолжат держаться в памяти. Вот три такие функции в коде ниже:
...
Но на практике движки JavaScript пытаются это оптимизировать. Они анализирует использование переменных и, если не сложно понять, что внешняя переменная не ипользуется -- она удаляется.
