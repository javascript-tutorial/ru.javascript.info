...
Но на практике движок JavaScript пытается это оптимизировать. Он анализирует использование переменных и, если легко по коду понять, что внешняя переменная не используется -- она удаляется.
