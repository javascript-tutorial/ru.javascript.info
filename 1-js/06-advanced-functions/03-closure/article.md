...
```smart header="Здесь мы поговорим о переменных `let/const`"
...
Для `if`, `for`, `while` и так далее. Переменные, объявленные в `{...}`, также видны только внутри:
...
Это здорово, так как позволяет нам создавать локальные переменные блока, специфичные для ветви `if`.
...
alert(i); // Error, нет такой переменной!
```
...
К примеру, в этом простом коде не содержащеего функций, только одно лексическое окружение:
