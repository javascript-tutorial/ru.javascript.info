...
1. В начале скрипта (блока кода, функции) лексическое окружение предварительно заполняется всеми объявленными переменными текущего блока кода.
    - Сперва эти переменные не проинициализированы. Это внутреннее состояние, значит, что движок знает о переменной, но к ней нельзя обратиться до момента объявления ее в коде. Можно считать, что этой переменной еще не существует (переменные, объявленные через var [ведут себя иначе](https://learn.javascript.ru/var#var-obrabatyvayutsya-v-nachale-zapuska-funktsii)). Обращение к переменной, которой нет в коде, выдасат ошибку `Uncaught ReferenceError: variableName is not defined`. Если же попроботь обратиться к существующей переменной до объявления, то будет ошибка `Uncaught ReferenceError: Cannot access 'variableName' before initialization`, что доказывает, что движок знает о нашей переменной.
