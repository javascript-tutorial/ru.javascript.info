...
Как это работает? Если мы создадим несколько счётчиков, будут ли они независимыми? Что здесь происходит с переменными?
...
Как бы мне ни хотелось избежать низкоуровневых деталей языка, любое объяснение без них будет недостаточным и неполным, так что приготовьтесь.
```

Для большей наглядности объяснение разбито на несколько этапов.
