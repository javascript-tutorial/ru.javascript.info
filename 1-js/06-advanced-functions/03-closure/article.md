...
До сих поры мы рассматривали только переменные. Теперь рассмотрим декларацию функции (Function Declaration).
...
Вот почему мы можем вызвать декларацию функции до того, как она определена.

Следующий код демонстрирует, что уже с самого начала в лексическом окружении что-то есть. Там есть `say`, потому что это декларация функции. И позже там появится `phrase`, объявленное через `let`:
...
1. Когда скрипт только начинает выполняться, есть только глобальное лексическое окружение:
    ...
    В этот начальный момент есть только функция `makeCounter`, потому что это декларация функции. Она ещё не выполняется.
...
3. В процессе выполнения `makeCounter()` создаётся небольшая вложенная функция.
    ...
    Не имеет значения, какой способ объявления функции используется: декларация функции или функциональное выражение. Все функции получают свойство `[[Environment]]`, которое ссылается на лексическое окружение, в которым они были созданы. Тоже происходит и с нашей новой маленькой функцией.
...
4. Так как выполнение продолжается, вызов `makeCounter()` заканчивается и результат (небольшая вложенная функция) присваивается глобальной переменной `counter`:
...
У него есть ссылка на внешнее окружение, так что `phrase` может быть найдена. Но все переменные и функциональные выражения, объявленные внутри `if`, остаются в его лексическом окружении и не видны снаружи.
...
Здесь создаётся и немедленно вызывается функциональное выражение. Так что код выполняется сразу же и у него есть свои приватные переменные.

Функциональное выражение обёрнуто в скобки `(function {...})`, потому что, когда JavaScript встречает `"function"` в основном потоке кода, он воспринимает это как начало декларации функции. Но у декларации функции должно быть имя, так что такой код вызовет ошибку:
...
Даже если мы скажем: "хорошо, давайте добавим имя", -- это не сработает, потому что JavaScript не позволяет вызывать декларацию функции немедленно.

```js run
// ошибка синтаксиса из-за скобок ниже
function go() {

}(); // <-- не можете вызывать декларацию функции немедленно
```
...
Кроме скобок, существуют и другие пути показать JavaScript, что мы имеем в виду функциональное выражение:
...
Во всех перечисленных случаях мы объявляем функциональное выражение и немедленно выполняем его.
