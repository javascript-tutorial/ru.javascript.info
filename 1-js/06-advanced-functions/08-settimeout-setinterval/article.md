...
Это сайд—эффект. Функция ссылается на внешнее лексическое окружение, поэтому пока она существует, внешние переменные существуют тоже. Они могут занимать больше памяти, чем сама функция. Поэтому если нет больше необходимости планирования вызова функции, то лучше отменить его, даже если он не очень большой.
````
...
Теперь интерфейс в браузере польностью доступен в течение всего процесса "подсчета".
