...
````smart header="Рекурсия в setTimeout"
Фактически рекурсии при вызове `setTimeout` из переданной функции нет. [Рекурсия](/recursion) – это вызов функцией самой себя. В данном случае новый `setTimeout` вызывает переданная функция. Разберём это на примере рекурсивного `setTimeout`:
...
Здесь вызов идёт из функции `run`, поэтому при рекурсивном вызове `setTimeout` [стек контекстов исполнения](/recursion) не будет увеличиваться и, соответственно, предела в глубине рекурсии здесь тоже не будет, как и нагрузки на память. Вот так будет работать код выше по шагам:

1. Вызов `setTimeout`, который планирует вызов функции `run` через 100 мс.
2. Исполнение функции `run` после задержки.
3. Вызов нового `setTimeout` из функции `run` для следующего планирования.
4. Завершение работы функции `run` и удаление его контекста исполнения из стека.
