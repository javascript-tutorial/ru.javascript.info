...
Удобно представить себе функции как вызываемые "объекты-действия". Мы можем их не только вызывать, но и рассматривать их как объекты: добавлять/удалять свойства, передавать их по ссылке и т.д.
...
Как мы видим, оставшиеся параметры не считаются.
...
Например, в коде ниже функция `ask` принимает параметр `question` чтобы задать вопрос, а также `handler`, произвольное количество функций-обработчиков.
...
Идея состоит в том, что у нас есть простой синтаксис обработчика без аргументов для положительных ответов (наиболее распространённый случай), а также реализована возможность передавать универсальные обработчики. 
...
Это частный случай так называемого [Ad-hoc-полиморфизма](https://ru.wikipedia.org/wiki/Полиморфизм_(информатика)#Ad-hoc-полиморфизм) -- обработка аргументов в зависимости от их типа или, как в нашем случае -- от значения. Эта идея имеет применение в библиотеках JavaScript.
...
Для начала отметим, что у нас все ещё есть Function Expression. Добавление `"func"` после `function` не превращает его в Function Declaration, потому что оно все еще является частью выражения присваивания.
...
Давайте зададим имя функции, чтобы исправить код:
...
- `length` -- количество аргументов в объявлении функции. Оставшиеся параметры не считаются.
...
Также, функции могут содержать дополнительные свойства. Многие широко известные JavaScript библиотеки искусно используют эту возможность.
