...
Наша первой темой будет *рекурсия*.

Если вы не новичок в программировании, то, возмоно, вы уже знакомы с рекурсией
и вы можете пропустить этоу главу.

Рекурсия - это паттерн программирования, полезеный в ситуациях, когда задача может быть естественно разделена на несколько аналогичных, но более простых задач. Или когда задача может быть упрощена в простое действие плюс более простой вариант той же задачи. Или, как мы скоро увидим, для работы с определенными структурами данных.
...
В качестве первого примера напишем функцию `pow(x, n)`, которая возводит `x` в натуральную степерь `n`. Иначе говоря, умножает `x` само на себя `n` раз.
...
2. Рекурсивный способ: упрощение задачи и вызов функции самой себя:
...
2. Мы можем представить `pow(x, n)` в виде: `x * pow(x, n - 1)`. Что в математике записывается как: <code>x<sup>n</sup> = x * x<sup>n-1</sup></code>. Эта ветвь -  *шаг рекурсии*: мы сводим задачу к более простому действию (умножение на `x`) и более простой аналогичной задаче (`pow` с меньшим `n`). Последующие шаги упрощают задачу всё больше, пока `n` не достигает `1`.
...
Общее количество вложенных вызовов (включая первый) называют *глубиной рекурсии*. В случае со степенью, всего будет ровно `n`.

Максимальная глубина рекурсии ограничена движком JavaScript. Точно можно рассчитывать на 10000 вложенных вызовов, некоторые интерпретаторы допускают и больше, но для большинства из них 100000 вызвов -- за прелелами возможностей. Существуют автоматические оптимизации, помогающие избежать переполнения стека вызовов ("оптимизация хвостовой рекурсии"), но они ещё не поддерживаются везде и работают только для простых случаев.

Это ограничивает применение рекурсии, но она все еще широко распространена: для решения большого числа задач рекурсивный способ решения дает более простой код, который легче обслуживать.
...
Информация о вызове функции храниться в её «контекстe выполнения».


[Контекст выполлнения](https://tc39.github.io/ecma262/#sec-execution-contexts) - специальная внутренняя структура данных, которая содержит информацию о вызове функции. Она включает в себя конкретное место в коде, на котором находится интерпретатор, локальные переменные функции, значение `this` (мы не используем его в данном примере) и прочию служебную информация.
...
- Контекст выполнения, связанный с ней запоминается в специальной структуре данных - *стек контекста выполнения*.
...
В начале вызова `pow(2, 3)` контекст выполения будет хранить переменные: `x = 2, n = 3`.
...
Начинается выполение функции. Условие `n == 1` ложно, поэтому выполняется вторая ветка условия `if`:
...
Для выполение вложенного вызова JavaScript запоминает текущий контекст выполения в *стеке контекста выполнения*.
...
Вид контекста в начале выполения вложенного вызова `pow(2, 2)`:
...
Новый контекст выполения находится на вершине стека (и выделен жирным), а предыдущие запомненные контексты -- под ним.

Когда выполение подвызова закончивается, можно будет легко вернуться назад, потому что контекст сохраняет как переменные, так и точное место кода, в котором он остановился. Слово «строка» на рисунках условно, на самом деле, конечно, запомнено более точное место в цепочке команд.
...
Когда функция заканчивается, контекст ее выполнения больше не нужен, поэтому он удаляется из памяти, из стека восстанавливается предыдущий:
...
Часто код с использованием рекурсии более короткий, легкий для понимания и поддержки. Оптимизация не требуется в каждом месте, в основном нам нужен хороший код, поэтому он и используется.
...
Представте, у нас есть компания. Структура персонала может быть представлена как объект:
...
- В отделе может состаять из массива работников. Например, в отделе `продаж`  работают 2 сотрудника: Джон и Элис.
- Или отдел может быть разделён на подразделы, как отдел `разработка` состоит из подразделов: `сайты` и `внутренее`. В каждом подразделе есть свой персонал.
- Также возможно, что при росте отдела он делится на подотделы (или команды).

    Например, отдел `сайты` в будущем может быть разделён на команды `сайтА` и `сайтБ`. И потенциально, они могут быть разделены ещё больше. Этого нет на картинке, просто нужно это иметь в виду.

Теперь, допустим, нам нужна функция для получения суммы всех зарплат. Как мы можем это сделать?

Итеративный подход не прост, потому что структура довольно сложная. Первая идея заключается в том, чтобы сделать цикл `for` поверх объекта компании` со вложенным подциклом над отделами 1-го уровня вложенности. Но затем нам нужно больше вложенных подциклов для итераций над сотрудниками отделов второго уровня, таких как `сайты`. ...а затем еще один подцикл внутри тех отделов 3-го уровня, которые могут появиться в будущем? Может, остановимся на третьем уровне или сделаем 4 уровня циклов? Если мы поместим в код 3-4 вложенных подцикла для обхода одного объекта, то это будет довольно некрасиво.

Давайте попробуем рекурсию.

Как мы видим, когда наша функция получает отдел для подсчёта сыммы зарплат, есть два возможных случая:

1. Либо это "простой" отдел с *массивом людей* - тогда мы сможем суммировать зарплаты в простом цикле.
2. Или это *объект с `N` подразделениями* - тогда мы можем сделать `N` рекурсивных вызовов, чтобы получить сумму для каждого из подотделов и объединить результаты.
...
- Метод `arr.reduce`, объяснённый в главе <info:array-methods>, для получения суммы элементов массива.
...
Для лучшего понимания мы рассмотрим еще одну рекурсивную структуру под названием "Связанный список", которая в некоторых случаях может использоваться в качестве альтернативы массивам.
...
...Но у массивов есть недостатки. Операции "удалить элемент" и "вставить элемент" являются дорогостоящими. Например, операция `arr.unhift(obj)` должна переиндексировать все элементы, чтобы освободить место для нового `obj`, и если массив большой, на это потребуется время. То же самое с `arr.shift()`.

Единственные струкурные изменения, не требующие массовой переиндексации, - это изменения, которые выполняются с конца массива: `arr.push/pop`. Таким образом, массив может быть довольно медленным для больших очередей.
...
- `value`,
- `next` - свойство, ссылающееся на следующий *элемент связанного списка* или `null`, если это послений элемент.
...
Здесь мы можем еще яснее видеть, что есть несколько объектов, каждый из которых имеет `value` и `next`, указывающий на соседа. Переменная `list` является первым объектом в цепочке, поэтому следуя по указателям `next` из нее, мы можем попасть в любой элемент.
...
Главным недостатком является то, что мы не можем легко получить доступ к элементу по его индексу. В простом массиве: `arr[n]` является прямой ссылкой. Но в списке мы должны начать с первого элемента и перейти в `next` N раз, чтобы получить N-ый элемент.
...
Иногда добавляют еще одну переменную под названием `tail` для отслеживания последнего элемента списка (и обновляют ее при добавлении/удалении элементов из конца списка). Для больших наборов элементов разница скоростей по сравнению с массивами огромна.

## Итого

Термины:
- *Рекурсия*  - это термин в программировании, означающий "самовызывающаяся" функция. Такие функции могут быть использованы для элегантного решения определенных задач.

    Когда функция вызывает саму себя, это называется *шагом рекурсии*. *База* рекурсии - это такие аргументы функции, которые делаяют задачу настолько простой, что решение не требует дальнейших вложенных вызовов.
...
- [Рекурсивно определяемая](https://en.wikipedia.org/wiki/Recursive_data_type) структура данных - это структура данных которая может быть определена с использованием самой себя.
    ...
    Например, связанный список может быть определен как структура данных, состоящая из объекта, содержащего ссылку на список (или нуль).
