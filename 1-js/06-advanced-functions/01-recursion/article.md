...
- Или отдел может быть разделён на подразделения, как отдел `разработка` состоит из подразделений: `сайты` и `внутренее`. В каждом подразделении есть свой персонал.
...
- [Рекурсивно определяемая](https://en.wikipedia.org/wiki/Recursive_data_type) структура данных - это структура данных, которая может быть определена с использованием самой себя.
    ...
    Например, связанный список может быть определён как структура данных, состоящая из объекта, содержащего ссылку на список (или нуль).
