# Rest parameters и Spread syntax
...
````warn header="Rest parameters должны располагаться в конце"
Rest parameters собирают все остальные аргументы, поэтому бессмысленно писать что-либо после них. Это вызовет ошибку:
...
К тому же, `arguments` всегда содержит все аргументы функции — мы не можем получить их часть. А Rest parameters позволяют это сделать.

Соответственно, для более удобной работы с аргументами лучше использовать Rest parameters.
...
И тут нам поможет *Spread syntax*. Он похож на Rest parameters – тоже использует `...`, но делает совершенно противоположное.
...
Мы даже можем комбинировать Spread syntax с обычными значениями:
...
Spread syntax можно использовать и для слияния массивов:
...
Например, Spread syntax подойдёт для того, чтобы превратить строку в массив символов:
...
Посмотрим, что происходит. Под капотом Spread syntax использует итераторы, чтобы перебирать элементы. Так же, как это делает `for..of`.
...
- Spread syntax работает только с итерируемыми объектами
...
Когда мы видим `"..."` в коде, это могут быть как Rest parameters, так и Spread syntax.

Как отличить их друг от друга:

- Если `...` располагается в конце списка аргументов функции, то это "Rest parameters". Он собирает остальные неуказанные аргументы и делает из них массив.
- Если `...` встретился в вызове функции или где-либо ещё, то это "Spread syntax". Он извлекает элементы из массива.

Полезно запомнить:

- Rest parameters используются, чтобы создавать новые функции с неопределённым числом аргументов.
