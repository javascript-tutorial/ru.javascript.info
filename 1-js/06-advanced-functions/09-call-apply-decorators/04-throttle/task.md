...
Когда он вызывается несколько раз, он передает вызов `f` максимум один раз в `ms` миллисекунд.
