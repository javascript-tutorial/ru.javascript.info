...
Аналогичный поиск в примере выше с `pattern:/\d\.\d/` вполне работал, почему же не работает с `new RegExp("\d\.\d")`?
...
- ...А когда нет особого значения: как например для `pattern:\d` или`\z`, обратная косая черта просто удаляется.
