...
Такой шаблон будет искать только `pattern:[01]\d` или `pattern:2[0-3]:[0-5]\d`.

Нам нужно, чтобы альтернация использовалась только внутри части регулярного выражения, относящейся к часам (`[01]\d` ИЛИ `2[0-3]`). Для этого обернём её в скобки: `pattern:([01]\d|2[0-3]):[0-5]\d`.
