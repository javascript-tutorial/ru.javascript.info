...
Одно и то же можно делать разными путями.

В этой главе мы группируем разные методы работы с регулярными выражениями по рецептам, чтобы было понятно, какую задачу как решать.

Если вы только начинаете изучать регулярные выражения, то можете пропустить её и вернуться позже, когда встретите незнакомый метод.

## Рецепты

**Для поиска всех совпадений:**

Используйте флаг `g` и методы:
- `str.match(reg)`, чтобы получить плоский массив совпадений,
- `str.matchAll(reg)`, получить массив совпадений с деталями (примеры будут позже).
