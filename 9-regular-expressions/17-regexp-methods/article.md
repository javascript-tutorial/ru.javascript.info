...
|`$n`|если `n` это 1-2 значное число, то вставляет содержимое n-й скобки, подробности см. в разделе [](info:regexp-groups)|
|`$<name>`|вставляет содержимое скобки с указанным `name`, подробности см. в разделе [](info:regexp-groups)|
|`$$`|вставляет `"$"` |
