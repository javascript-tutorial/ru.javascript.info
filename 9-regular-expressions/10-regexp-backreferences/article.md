...
Доступ к содержимому скобочных групп есть не только в результате поиска, но и в самом шаблоне.
...
```smart
Мы не можем обращаться к группе, если в ней используется `pattern:?:`. Группы, которые помечены `pattern:(?:...)`, не запоминаются и не участвуют в нумерации.
```
