...
При поиске `pattern:X(?=Y)` движок регулярных выражений, найдя `pattern:X`, проверяет есть ли после него `pattern:Y`. Если да - хорошо, если нет, то игнорирует совпадение и продолжает поиск дальше.
...
Такое возможно только при условии, что регулярные выражения `pattern:Y` и `pattern:Z` не являются взаимно исключающими.
...
Ещё пример, который демонстрирует, что движок регулярных выражений не берёт в совпадение результат проверки:

```js run
let str = "1 индейка стоит 30€";

alert( str.match(/\d+(?=\s)(?=.*30) индейка/) ); // 1 индейка
```

Здесь движок находит число `pattern:\d+`, дальше делает две проверки сразу после него и, когда они успешно проходят, идёт дальше и находит `pattern:индейка`.
...
Поэтому в примере выше примере совпадение с основным шаблоном `pattern:\d+` идёт первым, а результат для `pattern:(\$|£)` - вторым. Чтобы избежать путаницы, можно дать скобком имена, тогда эти особенности нумерации не имеют значения.
...
Как мы помним, `reg.match` (без флага `g`) и `str.matchAll` (всегда) возвращают совпадения со свойством `index`, которое содержит позицию совпадения в строке, так что мы можем посмотреть на контекст.
