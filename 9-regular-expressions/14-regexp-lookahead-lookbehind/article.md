...
Обычно порядок действия в случае наличия скобок -- это слева направо. Однако, ретроспективная проверка является исключением, её порядок захвата всегда идёт ПОСЛЕ главного шаблона. Поэтому в данном примере результат совпадения для `pattern:\d+` будет идти первым, а результат для `pattern:(\$|£)` будет вторым.
