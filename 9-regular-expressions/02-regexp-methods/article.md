...
alert( result[0] ); // JavaScript (полное совпадение)
alert( result[1] ); // Script (часть совпадения, соответствующая скобкам)
alert( result.index ); // 0
alert( result.input ); // JavaScript - это язык программирования
```
...
**Итак, с флагом `g` метод `str.match` возвращает простой массив всех совпадений без подробностей.**
...
Имейте это в виду, чтобы избежать ловушек, как в примере ниже:

```js run
let str = "Ой-йой-йой";

alert(str.match(/лю/gi).length) // Error: Cannot read property 'length' of null
```
...
Метод `str.matchAll(regexp)` используется для поиска всех совпадений с получением подробной информации.

Например:

```js run
let str = "Javascript или JavaScript? Нужна ли прописная буква 'S'?";
...
alert( match1[1] ); // script (Часть совпадения, которая соответсвует тому, что указано в шаблоне в круглых скобках)
...
Причина в том, что итератор не является массивом. Нам нужно преобразовать его через `Array.from (result)` или использовать цикл `for..of` для получения совпадений.

На практике, если нам нужны все совпадения, и `for..of` работает, так что это не проблема.

И, чтобы получить только несколько совпадений, мы можем использовать деструктуризацию:
...
Разбивает строку на массив по разделителю – регулярному выражению regexp или подстроке substr.
...
Чтобы найти все тире, нам нужно использовать не строку `"-"`, а шаблон регулярного выражения `/-/g` с обязательным флагом `g`:
...
| Спецсимвол | Действие в строке замены |
|--------|--------|
|`$$`|вставляет `"$"` |
...
// каждого John - заменить на Mr., а затем добавить John
...
В приведенном выше примере функция просто возвращает следующий номер каждый раз, но обычно результат основан на совпадении.
...
Метод `regexp.exec` является наиболее гибким методом поиска из всех. В отличие от предыдущих методов, `exec` должен вызываться в регулярном выражении, а не в строке.
