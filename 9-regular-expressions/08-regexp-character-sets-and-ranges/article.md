...
Кстати, заметим, что ищутся именно прописные буквы.
...
- `Mark` (`M`) - для акцентов (в Юникоде акценты могут быть представлены разными кодовыми позициями),
- `Decimal_Number` (`Nd`) - для цифр,
- `Connector_Punctuation` (`Pc`) - для символа `'_'` и подобных ему,
- `Join_Control` (`Join_C`) - две специальные кодовые позиции с кодами `200c` и `200d`, используемые в лигатурах, например, арабских.
...
Другими словами, разрешены все специальные символы, кроме случаев, когда они означают что-то особое в наборах.
...
Внутри `pattern:[…]` нужно экранировать:
- Закрывающую квадратную скобку `pattern:]` (если мы хотим найти такой символ)
- Дефис `pattern:-` на любой позиции, кроме первой и последней.
...
Причина в том, что обычно диапазон вида `pattern:[a-z]` - это "символы с кодами от `a` до `z`". Но без флага `pattern:u` суррогатные пары воспринимаются как два символа, так что  `[𝒳-𝒴]` воспринимается как `[<55349><56499>-<55349><56500>]` (каждая суррогатная пара заменена на коды). Теперь уже отлично видно, что диапазон `56499-55349` некорректен: его левая граница больше правой, это и есть причина ошибки.
