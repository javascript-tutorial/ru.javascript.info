...
Конечно, шаблон можно адаптировать: добавить юникодные свойства или убрать. Более подробно о них было рассказано в главе <infp:regexp-character-classes>.
...
- Дефис `pattern:-` на любой позиции, кроме первой и последней, то есть когда он означает диапазон.

## Диапазоны и флаг "u"

Если в диапазоне есть суррогатные пары, для корректной работы обязательно нужен флаг `pattern:u`.
