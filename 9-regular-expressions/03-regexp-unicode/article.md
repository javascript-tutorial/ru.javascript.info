...
В регулярном выражении можно искать символы с заданным свойствам, указав его в `pattern:\p{…}`. Для таких регулярных выражений обязательно использовать флаг `pattern:u`.
