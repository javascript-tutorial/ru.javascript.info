...
Как видно, это повторяющиеся слова, после каждого, кроме последнего, стоит точка.
...
Давайте это исправим, разрешив дефис `pattern:-` везде, кроме как в конце: `pattern:([\w-]+\.)+\w+`.
...
3. На позиции `2` - содержимое второй скобочнай группы.
4. ...и так далее...
...
На практике же часто возникает задача найти все сообщения, включая содержимое скобочных групп.

Для того, чтобы её решить, был создан метод `str.matchAll(regexp)`.

У него есть три отличия от `str.match(regexp)`:

1. Он возвращает не массив, а перебираемый объект.
2. При поиске с флагом `pattern:g`, он возвращает каждое совпадение в виде массива со скобочными группами не просто массив совпадений, а детали для каждого совпадения.
...
Как видите, первое отличие - очень важное. Если попытаться обратиться к `results[0]` напрямую, то будет ошибка. Объект результатов не является псевдомассивом. Его можно превратить в настоящий массив при помощи `Array.from`. Более подробно о псевдомассивах и перебираемых объектов мы говорили в главе <info:iterable>.
...
Метод `str.replace(regexp, replacement)`, осуществляющий замену совпадений с `regexp` в строке `str`, позволяет ссылаться на содержимое скобок при помощи обозначений вида `pattern:$n`, где `pattern:n` - номер скобочной группы.
...
- Метод `match` возвращает скобочные группы только без флага `g`.
- Метод `matchAll` возвращает скобочные группы всегда.
