...
Сейчас ему не соответствует домен с дефисом, например, `my-site.com`.

Давайте это исправим, разрешив дефис `pattern:-` везде, кроме как в конце: `pattern:([\w-]+\.)+\w+`.

Можно вносить и другие уточнения, но для исправления большинства опечаток такая проверка
...
## Все скобки и совпадения: matchAll
...
Если необходимы все совпадения со скобками, то мы можем использовать другой метод [matchAll](mdn:js/String/matchAll).

Метод `matchAll` аналогичен `match`, но есть два отличия:

1. Если есть флаг `pattern:g`, то он возвращает вместе с каждым совпадением содержимое скобочных групп.
...
alert(JSON.stringify(matchAll, null, 2));
/* каждое совпадение - массив с полным совпадением вначале и далее то, что в скобках
[
  [ "<h1>", "h1" ],
  [ "</h1>", "/h1"]
]
...
Для цикла по результатам в `Array.from` нет необходимости:
